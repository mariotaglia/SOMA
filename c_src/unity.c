...
#define UNITY_FAIL_AND_BAIL   { mpi_addon_err_msg_stop(); Unity.CurrentTestFailed  = 1; UNITY_OUTPUT_FLUSH(); TEST_ABORT(); }
...
#define FAIL_WITH_MPI_ADDON_MESSAGE(lineNumber, format, ...) {\
    UnityTestResultsFailBegin(lineNumber);\
    char err_msg[10000] = {'\0'};\
    sprintf(err_msg, format, __VA_ARGS__);\
    mpi_addon_write_err_msg(err_msg);\
    UNITY_FAIL_AND_BAIL;\
}

void UnityTrivialArrayCompare(size_t N, const void * expected, const void * actual, UNITY_LINE_TYPE line)
{
    if (N == 0)
        {
            FAIL_WITH_MPI_ADDON_MESSAGE(line, "comparing length %zu arrays is pointless", N);
        }
    if (expected == NULL)
        {
            FAIL_WITH_MPI_ADDON_MESSAGE(line, "expected array is %p", expected);
        }
    else if (actual == NULL)
        {
            FAIL_WITH_MPI_ADDON_MESSAGE(line, "expected non-null, actual array is %p", actual);
        }
}
...
#include <mpi.h>
void UnityAssertApproxUint16ArrayAbs(size_t N, const uint16_t * expected, const uint16_t * actual, uint16_t max_dev_elem, long long max_dev_sum, UNITY_LINE_TYPE line)
{
    UnityTrivialArrayCompare(N, expected, actual, line);

    long long expected_sum = 0;
    long long actual_sum = 0;
    for  (size_t i=0; i<N; i++)
    {
        if (ABS_DIFF(expected[i],actual[i]) > max_dev_elem)
        {
            FAIL_WITH_MPI_ADDON_MESSAGE(line, "array element %zu: expected: %d, actual %d (|actual-expected| > max_dev_elem=%d)", i, expected[i], actual[i], max_dev_elem);
        }
        expected_sum += expected[i];
        actual_sum += actual[i];
    }

    if (ABS_DIFF(expected_sum , actual_sum) > max_dev_sum)
    {
        DPRINT("failing bc of array sum")
        FAIL_WITH_MPI_ADDON_MESSAGE(line, "arrays are elementwise ok, but sum is wrong: expected %lld actual %lld (|actual-expected| > max_dev_sum=%lld", expected_sum, actual_sum, max_dev_sum);
    }
}
