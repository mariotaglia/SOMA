...
int init_MPI(struct Phase *p) {
  ...
  if (p->info_MPI.sim_size < p->args.N_domains_arg) {
    fprintf(
        stderr,
        "ERROR: Requested more domains %d than ranks are available %d. %s:%d\n",
  ...
  if (p->info_MPI.sim_size % p->args.N_domains_arg != 0) {
    fprintf(
        stderr,
        "ERROR: %s:%d invalid number of domains. #ranks mod Ndomains != 0.\n",
        __FILE__, __LINE__);
    return -7;
  }
  ...
  if (MPI_Comm_split(p->info_MPI.SOMA_comm_sim, domain_color, 0,
                     &(p->info_MPI.SOMA_comm_domain)) != MPI_SUCCESS) {
...
#if (ENABLE_MPI == 1)
  int *const status_array =
      (int *const)malloc(p->info_MPI.sim_size * sizeof(int));
  if (status_array == NULL) {
    fprintf(stderr, "ERROR: Malloc %s:%d \n", __FILE__, __LINE__);
    return -1;
  }
  MPI_Allgather(&my_status, 1, MPI_INT, status_array, 1, MPI_INT,
...
int collective_global_update(struct Phase *const p) {
  ...
  MPI_Allreduce(&(p->n_polymers), &(p->n_polymers_global), 1, MPI_UINT64_T,
                MPI_SUM, p->info_MPI.SOMA_comm_sim);
#pragma acc update device(p->n_polymers_global)
  // Total number of beads
  MPI_Allreduce(&(p->num_all_beads_local), &(p->num_all_beads), 1, MPI_UINT64_T,
                MPI_SUM, p->info_MPI.SOMA_comm_sim);
...
int send_polymer_chain(struct Phase *const p, const uint64_t poly_id,
  ...
  if (destination >= p->info_MPI.world_size) {
    fprintf(stderr,
            "ERROR: %s:%d World rank %d requested to send a polymer"
            " to world rank %d, but only world %d ranks are known.\n",
            __FILE__, __LINE__, p->info_MPI.world_rank, destination,
            p->info_MPI.world_rank);
    return -2;
  }
  if (p->n_polymers == 0) { // Send an empty buffer, to signal that no polymer
                            // is going to be send.
    unsigned int zero = 0;
  ...
  if (poly_id >= p->n_polymers) {
    fprintf(stderr,
            "ERROR: %s:%d World rank %d: Try to send poly_id %ld, but only %ld "
            "polymer local.\n",
  ...
  unsigned char *const buffer =
      (unsigned char *)malloc(buffer_length * sizeof(unsigned char));
  ...
  if (bytes_written != buffer_length) {
    fprintf(stderr,
            "ERROR: %s:%d world Rank %d: failed to serialize polymer %d %d\n",
            __FILE__, __LINE__, p->info_MPI.world_rank, bytes_written,
            buffer_length);
    return -3;
  }
...
int recv_polymer_chain(struct Phase *const p, const int source,
  ...
  if (source >= p->info_MPI.world_rank) {
    fprintf(stderr,
            "ERROR: %s:%d World rank %d requested to recv a polymer"
            " from w. rank %d, but only %d w. ranks are known.\n",
            __FILE__, __LINE__, p->info_MPI.world_rank, source,
            p->info_MPI.world_size);
    return -2;
  }
  ...
  if (buffer_length > 0) // The sending process can signal "no polymer to send."
  {
    ...
    MPI_Recv(buffer, buffer_length, MPI_UNSIGNED_CHAR, source, 0, comm,
             MPI_STATUS_IGNORE);
    ...
    if (err != (int)buffer_length) {
      fprintf(stderr,
              "ERROR: %s:%d World rank %d: invalid deserialization %d %d.\n",
  ...
  if (buffer_length > 0) {
    ...
    for (unsigned int i = 0; i < Nsends; i++) {
      ...
      const unsigned int poly_bytes =
          serialize_polymer(p, &poly, buffer + bytes_written);
      if (poly_bytes != poly_serial_length(p, &poly)) {
        fprintf(
            stderr,
            "ERROR: %s:%d:%d constructing invalid buffer construction %d,%d.\n",
            __FILE__, __LINE__, p->info_MPI.world_size, poly_bytes,
            poly_serial_length(p, &poly));
      }
      ...
      if (poly_len != poly_theo_len) {
        fprintf(stderr,
                "ERROR: %s:%d:%d invalid buffer length in constructed buffer "
                "%d %d\n",
                __FILE__, __LINE__, p->info_MPI.world_rank, poly_len,
                poly_theo_len);
        return -5;
      }
    ...
    // Send the buffer to destination.
    int err = MPI_Send(buffer, buffer_length, MPI_UNSIGNED_CHAR, destination, 2,
                       comm);
    if (err != MPI_SUCCESS) {
...
unsigned char *recv_mult_polymers_core(const int source, const MPI_Comm comm,
  ...
  if (*buffer_length >
      0) // The sending process can signal "no polymer to send."
  {
    // Allocate the memory to recv
    unsigned char *const buffer =
        (unsigned char *)malloc(*buffer_length * sizeof(unsigned char));
    ...
    MPI_Status status;
    // Recv the polymer buffer.
    int err = MPI_Recv(buffer, *buffer_length, MPI_UNSIGNED_CHAR, source, 2,
                       comm, &status);
    ...
    if ((unsigned int)recv_count != *buffer_length) {
      fprintf(stderr, "ERROR: %s:%d:%d message size differs %d %d\n", __FILE__,
              __LINE__, source, recv_count, *buffer_length);
      ret = NULL;
    }
    if (err != MPI_SUCCESS) {
...
int deserialize_mult_polymers(struct Phase *const p, const unsigned int Nsends,
  ...
  for (unsigned int i = 0; i < Nsends; i++) {
    ...
    if (poly_bytes > 0)
      ...
      fprintf(
          stderr,
          "ERROR: %s:%d rank %d invalid buffer length i=%d pb=%d \t %d %d %d\n",
          __FILE__, __LINE__, p->info_MPI.world_rank, i, poly_bytes, bytes_read,
          buffer_length, Nsends);
      return -1;
    }
  }
  ...
  if (bytes_read != buffer_length) {
    fprintf(stderr, "ERROR: %s:%d rank %d invalid buffer length %d %d\n",
            __FILE__, __LINE__, p->info_MPI.world_rank, bytes_read,
            buffer_length);
    return -1;
  }
  assert(bytes_read == buffer_length);
...
int recv_mult_polymers(struct Phase *const p, const int source,
  ...
  unsigned int buffer_length;
  unsigned char *const buffer =
      recv_mult_polymers_core(source, comm, &Nsends, &buffer_length);

  if (buffer != NULL) {
    const int err = deserialize_mult_polymers(p, Nsends, buffer_length, buffer);
    if (err != 0) {
      fprintf(stderr,
              "ERROR: %s:%d World rank %d: invalid deserialization %d.\n",
...
int load_balance_mpi_ranks(struct Phase *const p) {
  ...
  double *const waiting_time =
      (double *const)malloc(p->info_MPI.domain_size * sizeof(double));
  if (waiting_time == NULL) {
    fprintf(stderr, "ERROR: Malloc %s:%d\n", __FILE__, __LINE__);
    return -1;
  }
  double div = (p->info_MPI.domain_divergence_sec + 1) /
               (p->info_MPI.domain_divergence_counter + 1);
  ...
  const double max_p_waiting =
      passed_acc >= 0 && passed_acc <= 1 ? passed_acc : 0.08;
  ...
  if (p->info_MPI.domain_rank == arg_min) {
    ...
    copyout_phase(p);
    Nsend =
        send_mult_polymers(p, arg_max, Nchains, p->info_MPI.SOMA_comm_domain);
    consider_compact_polymer_heavy(p, false);
    copyin_phase(p);
  }
  ...
  if (p->info_MPI.domain_rank == 0)
    printf("INFO world rank %d: Load balance @t=%d, sending %d chains from "
           "domain rank"
           " %d to domain rank %d, because of %f percent waiting.\n",
           p->info_MPI.world_rank, p->time, Nsend, arg_min, arg_max,
           p_waiting * 100.);
...
/*! Extract all polymer chains out of the current system, if their center of
   mass is in another domain. \private The serialized buffers of the chains are
   returned for later communication. Depending on the input only neighboring
   domains or all domains are checked. \param p Phase describing the system
    \param domain_lookup_list List of domains, where chains can be send to.
    \param len_domain_list Lenght of the domain list.
    \param n_sends Array of len_domain_list, where the number of chains to be
   send to the corresponding domain is stored. \param buffer_len Array to store
   the length of the buffers for each domain \param buffer 2D Array where all
   buffers of the chains to be send are stored. \param my_domain Domain of the
   calling rank \return Number of chains, which should be transferred, but no
   valid domain was found. This is an Error if > 0.
*/
int extract_chains_per_domain(struct Phase *const p,
                              const int *domain_lookup_list,
                              const unsigned int len_domain_list,
                              unsigned int *const n_sends,
                              unsigned int *const buffer_len,
                              unsigned char **const buffer,
  ...
  unsigned int *const buffer_offsets =
      (unsigned int *const)malloc(len_domain_list * sizeof(unsigned int));
  ...
  for (unsigned int i = 0; i < len_domain_list; i++) {
    buffer_offsets[i] = 0;
    buffer[i] =
        (unsigned char *const)malloc(buffer_len[i] * sizeof(unsigned char));
      ...
      if (domain_index >= 0) {
        if ((unsigned int)domain_index >= len_domain_list) {
          fprintf(stderr, "ERROR %s:%d:%d invalid domain index %d %d\n",
                  __FILE__, __LINE__, p->info_MPI.world_rank, domain_index,
                  len_domain_list);
          return -2;
        }
        pop_polymer(p, i, &poly);
        i -= 1;
        const unsigned int poly_bytes = serialize_polymer(
            p, &poly, buffer[domain_index] + buffer_offsets[domain_index]);
        unsigned int poly_len;
  ...
  for (unsigned int i = 0; i < len_domain_list; i++) {
    if (buffer_offsets[i] != buffer_len[i]) {
      fprintf(stderr,
              "ERROR: %s:%d:%d Buffer construction inconsitent %d %d %d\n",
              __FILE__, __LINE__, p->info_MPI.world_rank, buffer_offsets[i],
              buffer_len[i], i);
      return -1;
    }
  }
  ...
  if (chains_missed != 0)
    printf("ERROR: %s:%d: World rank %d has to send %d chains to a non "
           "neighbor rank. Restart simulation with higher rcm update rate.\n",
...
int send_domain_chains(struct Phase *const p, const bool init) {
  ...
  unsigned int *const domain_list =
      (unsigned int *const)malloc(len_domain_list * sizeof(unsigned int));
  MALLOC_ERROR_CHECK(domain_list, len_domain_list * sizeof(unsigned int));
  int *const domain_lookup_list =
      (int *const)malloc(p->args.N_domains_arg * sizeof(int));
  ...
  if (init) {
    ...
    domain_list[0] = my_domain == 0 ? (unsigned int)p->args.N_domains_arg - 1
                                    : my_domain - 1;
    domain_list[1] =
        (int)my_domain == p->args.N_domains_arg - 1 ? 0 : my_domain + 1;
  }
  ...
  const int missed_chains =
      extract_chains_per_domain(p, domain_lookup_list, len_domain_list, n_sends,
                                send_len, send_buffer, my_domain);

  MPI_Request *const req =
      (MPI_Request *const)malloc(4 * len_domain_list * sizeof(MPI_Request));
  MALLOC_ERROR_CHECK(req, 4 * len_domain_list * sizeof(MPI_Request));
  MPI_Status *const status =
      (MPI_Status *const)malloc(4 * len_domain_list * sizeof(MPI_Status));
  ...
  for (unsigned int i = 0; i < len_domain_list; i++) {
    ...
    const unsigned int comm_to =
        comm_domain * p->info_MPI.domain_size + p->info_MPI.domain_rank;
    MPI_Isend(n_sends + i, 1, MPI_UNSIGNED, comm_to, 0,
              p->info_MPI.SOMA_comm_sim, req + (0 * len_domain_list + i));
    MPI_Isend(send_len + i, 1, MPI_UNSIGNED, comm_to, 1,
              p->info_MPI.SOMA_comm_sim, req + (1 * len_domain_list + i));

    MPI_Irecv(n_recv + i, 1, MPI_UNSIGNED, comm_to, 0,
              p->info_MPI.SOMA_comm_sim, req + (2 * len_domain_list + i));
    MPI_Irecv(recv_len + i, 1, MPI_UNSIGNED, comm_to, 1,
              p->info_MPI.SOMA_comm_sim, req + (3 * len_domain_list + i));
  }
  ...
  for (unsigned int i = 0; i < len_domain_list; i++) {
    recv_buffer[i] =
        (unsigned char *)malloc(recv_len[i] * sizeof(unsigned char));
  ...
  for (unsigned int i = 0; i < len_domain_list; i++) {
    ...
    const unsigned int comm_to =
        comm_domain * p->info_MPI.domain_size + p->info_MPI.domain_rank;
    MPI_Isend(send_buffer[i], send_len[i], MPI_UNSIGNED_CHAR, comm_to, 2,
              p->info_MPI.SOMA_comm_sim, req + (0 * len_domain_list + i));

    MPI_Irecv(recv_buffer[i], recv_len[i], MPI_UNSIGNED_CHAR, comm_to, 2,
              p->info_MPI.SOMA_comm_sim, req + (1 + len_domain_list + i));
    ...
    if (recv_buffer[i] != NULL) {
      ...
      if (err != 0) {
        fprintf(stderr,
                "ERROR: %s:%d World rank %d: invalid deserialization %d. "
                " i=%d\t %d %d %p\n",
                __FILE__, __LINE__, p->info_MPI.world_rank, err, i, n_recv[i],
                recv_len[i], recv_buffer[i]);
        break;
      }
    } else {
      assert(n_recv[i] == 0);
