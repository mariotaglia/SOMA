...
int init_phase(struct Phase *const p)
{
    ...
    if (p->args.N_domains_arg > 1 && p->info_MPI.domain_rank == 0)
        {
            ...
            p->left_tmp_buffer =
	      (uint16_t *) malloc( (p->args.domain_buffer_arg * p->ny * p->nz + safety_32_16_margin) * sizeof(uint16_t) );
    ...
    p->fields_unified = (uint16_t *) malloc( (p->n_cells_local * p->n_types + safety_32_16_margin)* sizeof(uint16_t) );
