...
#pragma omp target update from(p->polymers[0:p->n_polymers])
...
#pragma omp target update from(p->old_fields_unified[0:p->n_types*p->n_cells_local])
#pragma acc update self(p->fields_32[0:p->n_types*p->n_cells_local])
#pragma omp target update from(p->fields_32[0:p->n_types*p->n_cells_local])

    if (p->area51 != NULL)
        {
#pragma acc update self(p->area51[0:p->n_cells_local])
#pragma omp target update from(p->area51[0:p->n_cells_local])
        }
...
#pragma omp target update\
            from(p->external_field_unified[0:p->n_cells_local*p->n_types])
        }
    if (p->umbrella_field != NULL)
        {
#pragma acc update self(p->umbrella_field[0:p->n_cells_local*p->n_types])
#pragma omp target update from(p->umbrella_field[0:p->n_cells_local*p->n_types])
        }

#pragma acc update self(p->tempfield[0:p->n_cells_local])
#pragma omp target update from(p->tempfield[0:p->n_cells_local])
...
#pragma omp target update from(p->k_umbrella[0:p->n_types])
...
// Code was translated using: /p/project/training2215/tools/intel-acc-to-omp/src/intel-acc-to-omp -force-backup phase.c
