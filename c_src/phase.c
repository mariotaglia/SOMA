...
int init_phase(struct Phase *const p) {
...
#if (ENABLE_MPI == 1)
  MPI_Scan(&(p->n_polymers), &n_polymer_offset, 1, MPI_UINT64_T, MPI_SUM,
  ...
  case pseudo_random_number_generator_arg_MT:
    init_soma_memory(&(p->rh.mt), p->n_polymers,
  ...
  for (uint64_t i = 0; i < p->n_polymers; i++) {
    ...
    case pseudo_random_number_generator_arg_PCG32:
      ...
      break;
    case pseudo_random_number_generator_arg_MT:
      p->polymers[i].poly_state.alternative_rng_offset =
    ...
    seed_rng_state(&(p->polymers[i].poly_state), p->args.rng_seed_arg,
                   i + n_polymer_offset, p);
  }

  // Max safe move distance
  p->max_safe_jump =
      p->Lx / p->nx < p->Ly / p->ny ? p->Lx / p->nx : p->Ly / p->ny;
  p->max_safe_jump =
      p->max_safe_jump < p->Lz / p->nz ? p->max_safe_jump : p->Lz / p->nz;
  ...
  MPI_Allreduce(&(p->n_polymers), &n_polymers_global_sum, 1, MPI_UINT64_T,
  ...
  p->local_nx_low =
      (p->nx / p->args.N_domains_arg * my_domain) - p->args.domain_buffer_arg;
  p->local_nx_high = p->local_nx_low + p->nx / p->args.N_domains_arg +
                     2 * p->args.domain_buffer_arg;
  ...
  if (p->args.N_domains_arg > 1 && p->info_MPI.domain_rank == 0) {
    p->left_tmp_buffer = (uint16_t *)malloc(
        (p->args.domain_buffer_arg * p->ny * p->nz) * sizeof(uint16_t));
    if (p->left_tmp_buffer == NULL) {
      fprintf(stderr, "ERROR: Malloc %s:%d\n", __FILE__, __LINE__);
      return -1;
    }
    p->right_tmp_buffer = (uint16_t *)malloc(
        (p->args.domain_buffer_arg * p->ny * p->nz) * sizeof(uint16_t));
  ...
  p->fields_unified =
      (uint16_t *)malloc(p->n_cells_local * p->n_types * sizeof(uint16_t));
  p->fields_unified =
      (uint16_t *)malloc((p->n_cells_local * p->n_types) * sizeof(uint16_t));
  ...
  p->fields_32 =
      (uint32_t *)malloc(p->n_types * p->n_cells_local * sizeof(uint32_t));
  if (p->fields_32 == NULL) {
    fprintf(stderr, "ERROR: Malloc %s:%d\n", __FILE__, __LINE__);
    return -1;
  }
  p->omega_field_unified = (soma_scalar_t *)malloc(
      p->n_cells_local * p->n_types * sizeof(soma_scalar_t));
  ...
  for (uint64_t j = 0; j < p->n_polymers; j++) { /*Loop over polymers */
    const unsigned int N =
        p->poly_arch[p->poly_type_offset[p->polymers[j].type]];
  ...
  // setting the geometry field, or if it's already initialized measure the free
  // space
  uint64_t ncells = (p->nx / p->args.N_domains_arg) * p->ny * p->nz;
  if (p->info_MPI.domain_rank ==
      0) // Only domain root calculats something else than 0
  {
    if (p->area51 != NULL) {
...
#if (ENABLE_MPI == 1)
  MPI_Allreduce(MPI_IN_PLACE, &ncells, 1, MPI_UINT64_T, MPI_SUM,
                p->info_MPI.SOMA_comm_sim);
#endif // ENABLE_MPI

  p->n_accessible_cells = ncells;
  // Loop to calculate scaling parameter
  // Note the *= the field is initialized with the density weights in
  // read_hdf5_config. default value = 1.
  ...
  if (p->args.coord_file_arg !=
      NULL) // This is only set to NULL if convert is used.
  {
...
int copyin_phase(struct Phase *const p) {
  if (p->present_on_device) {
    fprintf(stderr,
            "WARNING: %s:%d copyin of the phase, but system seems to be "
...
#pragma acc enter data copyin(p->fields_unified                                \
                              [0:(p->n_types * p->n_cells_local)])
#pragma acc enter data copyin(p->old_fields_unified                            \
                              [0:(p->n_types * p->n_cells_local)])
...
#pragma acc enter data copyin(p->omega_field_unified                           \
                              [0:p->n_cells_local * p->n_types])
  if (p->external_field_unified != NULL) {
#pragma acc enter data copyin(p->external_field_unified                        \
                              [0:p->n_cells_local * p->n_types])
  }
  if (p->umbrella_field != NULL) {
#pragma acc enter data copyin(p->umbrella_field                                \
                              [0:p->n_cells_local * p->n_types])
  }
...
#pragma acc enter data copyin(p->sets[i].sets                                  \
                              [0:p->sets[i].n_sets * p->sets[i].max_member])
    }
  }
#ifdef ENABLE_MPI_CUDA
  // in this case also copy in the buffers:

#pragma acc enter data copyin(p->left_tmp_buffer                               \
                              [0:(p->args.domain_buffer_arg * p->ny * p->nz)])
#pragma acc enter data copyin(p->right_tmp_buffer                              \
                              [0:(p->args.domain_buffer_arg * p->ny * p->nz)])
...
int copyout_phase(struct Phase *const p) {
  if (!p->present_on_device) {
    fprintf(stderr,
            "WARNING: %s:%d copyout of the phase, but system seems to be not "
            "present on device.\n",
            __FILE__, __LINE__);
  }
#ifdef _OPENACC
...
#pragma acc exit data copyout(p->fields_unified                                \
                              [0:p->n_types * p->n_cells_local])
#pragma acc exit data copyout(p->old_fields_unified                            \
                              [0:p->n_types * p->n_cells_local])
...
#pragma acc exit data copyout(p->omega_field_unified                           \
                              [0:p->n_cells_local * p->n_types])
  if (p->external_field_unified != NULL) {
#pragma acc exit data copyout(p->external_field_unified                        \
                              [0:p->n_cells_local * p->n_types])
  }
  if (p->umbrella_field != NULL) {
#pragma acc exit data copyout(p->umbrella_field                                \
                              [0:p->n_cells_local * p->n_types])
  }
...
#pragma acc exit data copyout(p->sets[i].sets                                  \
                              [0:p->sets[i].n_sets * p->sets[i].max_member])
    }
#pragma acc exit data copyout(p->sets [0:p->n_poly_type])
  }
#ifdef ENABLE_MPI_CUDA
#pragma acc exit data copyout(p->left_tmp_buffer                               \
                              [0:p->args.domain_buffer_arg * p->ny * p->nz])
#pragma acc exit data copyout(p->right_tmp_buffer                              \
                              [0:p->args.domain_buffer_arg * p->ny * p->nz])
#endif // ENABLE_MPI_CUDA
...
int free_phase(struct Phase *const p) {
  ...
  if (p->args.coord_file_arg !=
      NULL) // Required option. So always true, if args used.
...
#pragma acc update self(p->omega_field_unified                                 \
                        [0:p->n_cells_local * p->n_types])
  if (p->external_field_unified != NULL) {
#pragma acc update self(p->external_field_unified                              \
                        [0:p->n_cells_local * p->n_types])
  }
  if (p->umbrella_field != NULL) {
...
int mc_set_init(Phase *const p) {
  ...
  unsigned int *poly_order =
      (unsigned int *)malloc((int)p->n_polymers * sizeof(unsigned int));
  if (poly_order == NULL) {
    fprintf(stderr, "ERROR: malloc %s:%d\n", __FILE__, __LINE__);
    return -1;
  }
  ...
  for (uint64_t poly_i = 1; poly_i < p->n_polymers; poly_i++) {
    ...
    while (i >= 0 &&
           length_poly_i >
               p->poly_arch
                   [p->poly_type_offset[(&p->polymers[poly_order[i]])->type]])
      i--;

    i = poly_i - 1 - i;
    if (i != 0)
      memmove(poly_order + poly_i - i, poly_order + poly_i - i + 1,
