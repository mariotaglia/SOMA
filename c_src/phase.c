...
int init_phase(struct Phase *const p)
{
    ...
    p->d_invblav = (soma_scalar_t *) malloc(p->n_cells_local * p->n_types * sizeof(soma_scalar_t));
    if (p->d_invblav == NULL)
        {
            fprintf(stderr, "ERROR: Malloc %s:%d\n", __FILE__, __LINE__);
            return -1;
    ...
    p->exp_born_pol = (soma_scalar_t *) malloc(p->n_cells_local * sizeof(soma_scalar_t));
    if (p->exp_born_pol == NULL)
        {
            fprintf(stderr, "ERROR: Malloc %s:%d\n", __FILE__, __LINE__);
            return -1;
        }

    p->exp_born_pos = (soma_scalar_t *) malloc(p->n_cells_local * sizeof(soma_scalar_t));
    if (p->exp_born_pos == NULL)
        {
            fprintf(stderr, "ERROR: Malloc %s:%d\n", __FILE__, __LINE__);
            return -1;
        }

    p->exp_born_neg = (soma_scalar_t *) malloc(p->n_cells_local * sizeof(soma_scalar_t));
    if (p->exp_born_neg == NULL)
        {
            fprintf(stderr, "ERROR: Malloc %s:%d\n", __FILE__, __LINE__);
            return -1;
        }

    p->born_Sc = (soma_scalar_t *) malloc(p->n_cells_local * sizeof(soma_scalar_t));
    if (p->born_Sc == NULL)
        {
            fprintf(stderr, "ERROR: Malloc %s:%d\n", __FILE__, __LINE__);
            return -1;
   ...
   if (p->args.efieldsolver_arg != efieldsolver_arg_NO)  { // Info for electrostatics, prepare data before copyin
             ...
             initc(p);  // init concentrations of ion concentrations to homogeneous solution
    }
...
#pragma acc enter data copyin(p->born_Sc[0:p->n_cells])
