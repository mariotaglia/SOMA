...
void self_omega_field(const struct Phase *const p)
    {
      ...
      // for(int c=0;c<(p->n_types * p->n_cells);c++)
...
#pragma omp parallel for
	for (uint64_t cell = 0; cell < p->n_cells; cell++)/*Loop over all cells, max number of cells is product of nx, ny,nz */
	    {
	    p->omega_field_unified[cell + T_types*p->n_cells] = inverse_refbeads * (p->xn[T_types][T_types] * (p->tempfield[cell] - 1.0));
...
#pragma omp parallel for
		...
		soma_scalar_t interaction = dnorm *
		    (  p->field_scaling_type[T_types]*p->fields_unified[cell+T_types*p->n_cells]
		       - p->field_scaling_type[S_types]*p->fields_unified[cell+S_types*p->n_cells]); /*Added the rescaling cause p->fields are short now*/
