...
#pragma omp target distribute parallel for order(concurrent) map(always,alloc:p[0:1])
...
//#pragma omp loop 
//OMP: this definitely needs some more attention. Here we will loose performance.
...
#pragma acc parallel loop present(p[:1])
#pragma omp target distribute parallel for map(always,alloc:p[:1])
#ifdef _OPENMP_CPU
#pragma omp parallel for
#endif                   //_OPENMP_CPU
                    for (uint64_t cell = 0; cell < p->n_cells_local; cell++)
