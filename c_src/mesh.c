...
#pragma acc update self(p->fields_unified [0:p->n_cells_local * p->n_types])
  MPI_Allreduce(MPI_IN_PLACE, p->fields_unified, p->n_cells_local * p->n_types,
                MPI_UINT16_T, MPI_SUM, p->info_MPI.SOMA_comm_sim);
...
#pragma acc host_data use_device(fields_32)
    {
      ...
      ncclAllReduce(fields_32, fields_32, p->n_cells_local * p->n_types,
                    ncclUint32, ncclSum, p->info_MPI.SOMA_nccl_sim,
                    acc_get_cuda_stream(acc_get_default_async()));
    }
#pragma acc wait
    copy_density_32_to_16(p);
  } else {
    MPI_Allreduce(MPI_IN_PLACE, p->fields_unified,
                  p->n_cells_local * p->n_types, MPI_UINT16_T, MPI_SUM,
...
#pragma acc host_data use_device(fields_unified)
  {
    MPI_Allreduce(MPI_IN_PLACE, fields_unified, p->n_cells_local * p->n_types,
                  MPI_UINT16_T, MPI_SUM, p->info_MPI.SOMA_comm_sim);
  }
#endif // ENABLE_NCCL

  //#pragma acc update self(p->fields_unified[0:p->n_cells_local*p->n_types])
  // No update of data on cpu required since it is only needed for analytics
...
#pragma acc host_data use_device(fields_32)
    {
      ncclReduce(fields_32, fields_32, p->n_cells_local * p->n_types,
                 ncclUint32, ncclSum, 0, p->info_MPI.SOMA_nccl_domain,
                 acc_get_cuda_stream(acc_get_default_async()));
    }
#pragma acc wait
    copy_density_32_to_16(p);
    ...
    if (p->info_MPI.domain_rank == 0)
      MPI_Reduce(MPI_IN_PLACE, p->fields_unified, p->n_cells_local * p->n_types,
                 MPI_UINT16_T, MPI_SUM, 0, p->info_MPI.SOMA_comm_domain);
    else
      MPI_Reduce(p->fields_unified, NULL, p->n_cells_local * p->n_types,
                 MPI_UINT16_T, MPI_SUM, 0, p->info_MPI.SOMA_comm_domain);
  }
#endif // ENABLE_NCCL
...
#pragma acc update self(p->fields_unified [0:p->n_cells_local * p->n_types])
#else // ENABLE_MPI_CUDA
#pragma acc host_data use_device(fields_unified, left_tmp_buffer,              \
                                 right_tmp_buffer)
  {
#endif // ENABLE_MPI_CUDA
  ...
  if (p->info_MPI.domain_rank == 0)
    MPI_Reduce(MPI_IN_PLACE, fields_unified, p->n_cells_local * p->n_types,
               MPI_UINT16_T, MPI_SUM, 0, p->info_MPI.SOMA_comm_domain);
  else
    MPI_Reduce(fields_unified, NULL, p->n_cells_local * p->n_types,
               MPI_UINT16_T, MPI_SUM, 0, p->info_MPI.SOMA_comm_domain);
#endif // ENABLE_NCCL
  ...
  if (p->info_MPI.domain_rank == 0) {
    ...
    MPI_Request req[4];
    MPI_Status stat[4];
    // Loop over type, because of the memory layout [type][x][y][z] -> x is not
    ...
    for (unsigned int type = 0; type < p->n_types; type++) {
      ...
      MPI_Isend(fields_unified + (p->n_cells_local - ghost_buffer_size) +
                ...
                ghost_buffer_size, MPI_UINT16_T, right_rank, 0,
      ...
      MPI_Isend(fields_unified + type * p->n_cells_local, ghost_buffer_size,
                MPI_UINT16_T, left_rank, 1, p->info_MPI.SOMA_comm_sim, req + 2);
      MPI_Irecv(right_tmp_buffer, ghost_buffer_size, MPI_UINT16_T, right_rank,
                1, p->info_MPI.SOMA_comm_sim, req + 3);
...
#pragma acc parallel loop present(p [0:1])
      ...
      MPI_Isend(fields_unified + (p->n_cells_local - 2 * ghost_buffer_size) +
                    type * p->n_cells_local,
                ghost_buffer_size, MPI_UINT16_T, right_rank, 2,
                p->info_MPI.SOMA_comm_sim, req + 0);
      ...
      MPI_Irecv(fields_unified + (p->n_cells_local - ghost_buffer_size) +
                    type * p->n_cells_local,
                ghost_buffer_size, MPI_UINT16_T, right_rank, 3,
...
/*! Function to communicate the density field accross all ranks.
  If a domain decomposition is used, the communication pattern is accordingly
  adopted. \param p Phase describing the system.
*/
...
#pragma omp parallel for
  for (uint64_t index = 0; index < n_indices;
       index++) /*Loop over all fields according to monotype */
    p->fields_32[index] = 0;
  const uint64_t n_polymers = p->n_polymers;

#pragma acc parallel loop gang num_gangs(n_polymers) vector_length(128)        \
    present(p [0:1])
#pragma omp parallel for
...
#pragma acc loop vector
    for (unsigned int j = 0; j < N; j++) { /*Loop over monomers */
      ...
      const unsigned int index = coord_to_index_unified(
          p, beads[j].x, beads[j].y, beads[j].z, monotype);
...
#pragma acc exit data copyout(error_flags [0:1])
  if (error_flags[0] != 0) {
    fprintf(stderr,
            "ERROR: Domain error. %d world-rank %d"
  ...
  copy_density_32_to_16(p);
  /*Share the densityfields -> needed because Hamiltonian may not only quadratic
   * order */
...
#pragma omp parallel for
    for (uint64_t cell = 0; cell < p->n_cells_local; cell++)
      p->tempfield[cell] +=
          rescale_density *
          p->fields_unified[T_types * p->n_cells_local + cell];
...
void self_omega_field(const struct Phase *const p) {
  ...
  /*Tempfields save the complete densities and remain as type soma_scalar_t ->
   * used for insothermal Compressibility */
  ...
  if (p->time == 0 || p->serie_length == 0) { // to fix serie_length seg fault
    ...
    for (unsigned int serie_index = 0; serie_index < p->serie_length;
         serie_index++) {
      external_field_time += p->cos_serie[serie_index] *
                             cos(2 * M_PI * serie_index / p->period * p->time);
      external_field_time += p->sin_serie[serie_index] *
                             sin(2 * M_PI * serie_index / p->period * p->time);
    }
  }
  // Compressibility part + external fields
  for (unsigned int T_types = 0; T_types < p->n_types;
       T_types++) /*Loop over all fields according to monotype */
  {
#pragma acc parallel loop present(p[:1])
#pragma omp parallel for
    for (uint64_t cell = 0; cell < p->n_cells_local;
         cell++) /*Loop over all cells, max number of cells is product of nx,
                    ny,nz */
    {
      p->omega_field_unified[cell + T_types * p->n_cells_local] =
          inverse_refbeads *
          (p->xn[T_types * p->n_types + T_types] * (p->tempfield[cell] - 1.0));
      ...
      if (p->external_field_unified != NULL) {
        p->omega_field_unified[cell + T_types * p->n_cells_local] +=
            inverse_refbeads *
            p->external_field_unified[cell + T_types * p->n_cells_local] *
            external_field_time;
      }
      // umbrella part
...
void add_pair_omega_fields_scmf0(const struct Phase *const p) {
  ...
  for (unsigned int T_types = 0; T_types < p->n_types;
    ...
    for (unsigned int S_types = T_types + 1; S_types < p->n_types; S_types++) {
      ...
      soma_scalar_t dnorm =
          -0.5 * inverse_refbeads * p->xn[T_types * p->n_types + S_types];
#pragma acc parallel loop present(p[:1])
#pragma omp parallel for
      for (uint64_t cell = 0; cell < p->n_cells_local; cell++) {
        soma_scalar_t interaction =
            dnorm * (p->field_scaling_type[T_types] *
                     ...
                     p->field_scaling_type[S_types] *
                         p->fields_unified[cell + S_types * p->n_cells_local]);
        p->omega_field_unified[cell + T_types * p->n_cells_local] +=
            interaction;
        p->omega_field_unified[cell + S_types * p->n_cells_local] -=
            interaction;
      }
    }
  }
...
#pragma omp parallel for
      for (uint64_t cell = 0; cell < p->n_cells_local; cell++) {
        const soma_scalar_t normT =
            inverse_refbeads * p->xn[T_types * p->n_types + S_types];
        const soma_scalar_t rhoS =
            p->fields_unified[cell + S_types * p->n_cells_local] *
            p->field_scaling_type[S_types];
        const soma_scalar_t normS =
            inverse_refbeads * p->xn[S_types * p->n_types + T_types];
