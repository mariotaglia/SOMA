...
void calc_Re(const struct Phase *p, soma_scalar_t *const result) {
  uint64_t *const counter =
      (uint64_t *)malloc(p->n_poly_type * sizeof(uint64_t));
  if (counter == NULL) {
...
#if (ENABLE_MPI == 1)
  MPI_Allreduce(MPI_IN_PLACE, result, 4 * p->n_poly_type, MPI_SOMA_SCALAR,
...
void calc_dvar(const struct Phase *p, soma_scalar_t *dvar) {
  uint64_t var = 0.0;
  if (p->info_MPI.domain_rank ==
      0) // Only the root domain rank needs to calculate the value
  {
    const unsigned int my_domain =
        p->info_MPI.sim_rank / p->info_MPI.domain_size;
        ...
        for (unsigned int y = 0; y < p->ny; y++)
          for (unsigned int z = 0; z < p->nz; z++) {
            const uint64_t index =
                cell_coordinate_to_index(p, x, y, z) + type * p->n_cells_local;
            const uint64_t value =
                p->fields_unified[index] - p->old_fields_unified[index];
            var += value * value;
          }
  }
  memcpy(p->old_fields_unified, p->fields_unified,
         p->n_cells_local * p->n_types * sizeof(uint16_t));
  *dvar = var; // Cast to float
  *dvar /= p->n_cells_local * p->n_types;
#if (ENABLE_MPI == 1)
  if (p->info_MPI.sim_rank == 0)
    MPI_Reduce(MPI_IN_PLACE, dvar, 1, MPI_SOMA_SCALAR, MPI_SUM, 0,
               p->info_MPI.SOMA_comm_sim);
  else
    MPI_Reduce(dvar, NULL, 1, MPI_SOMA_SCALAR, MPI_SUM, 0,
               p->info_MPI.SOMA_comm_sim);
#endif // ENABLE_MPI
  ...
  for (uint64_t ipoly = 0; ipoly < p->n_polymers; ipoly++) {
    ...
    for (unsigned int ibead = 0; ibead < N; ibead++) {
      ...
      const int start = get_bondlist_offset(
          p->poly_arch[p->poly_type_offset[type] + ibead + 1]);
      if (start > 0) {
        int end = 0;
...
#if (ENABLE_MPI == 1)
  MPI_Allreduce(MPI_IN_PLACE, result, 6 * p->n_poly_type, MPI_SOMA_SCALAR,
                MPI_SUM, p->info_MPI.SOMA_comm_sim);
  MPI_Allreduce(MPI_IN_PLACE, counter, p->n_poly_type, MPI_UINT64_T, MPI_SUM,
                p->info_MPI.SOMA_comm_sim);
#endif // ENABLE_MPI
...
void calc_MSD(const struct Phase *p, soma_scalar_t *const result) {
  uint64_t *const counter =
      (uint64_t *)malloc(2 * p->n_poly_type * sizeof(uint64_t));
  if (counter == NULL) {
    fprintf(stderr, "MALLOC ERROR: %s:%d\n", __FILE__, __LINE__);
    return;
  }
...
#if (ENABLE_MPI == 1)
  MPI_Allreduce(MPI_IN_PLACE, result, 8 * p->n_poly_type, MPI_SOMA_SCALAR,
                MPI_SUM, p->info_MPI.SOMA_comm_sim);
  MPI_Allreduce(MPI_IN_PLACE, counter, 2 * p->n_poly_type, MPI_UINT64_T,
                MPI_SUM, p->info_MPI.SOMA_comm_sim);
#endif // ENABLE_MPI

  // Looping over twice the number of poly types. But loop over half the
  // elements
...
void calc_acc_ratio(struct Phase *const p, soma_scalar_t *const acc_ratio) {
  ...
  *acc_ratio = -1;
  ...
  else { // Quick exit, because the property has already been calculated for the
         // time step.
    *acc_ratio = last_acc;
    return;
  }
...
#if (ENABLE_MPI == 1)
  MPI_Reduce(exchange + 2, exchange, 2, MPI_UINT64_T, MPI_SUM, 0,
             p->info_MPI.SOMA_comm_sim);
  if (exchange[1] > 0)
...
void calc_non_bonded_energy(const struct Phase *const p,
                            soma_scalar_t *const non_bonded_energy) {
  memset(non_bonded_energy, 0, p->n_types * sizeof(soma_scalar_t));

  if (p->info_MPI.domain_rank ==
      0) // Only the root domain rank needs to calculate the values.
  {
    const unsigned int my_domain =
        p->info_MPI.sim_rank / p->info_MPI.domain_size;
    for (unsigned int type = 0; type < p->n_types; type++) {
...
#if (ENABLE_MPI == 1)
  if (p->info_MPI.sim_rank == 0)
    MPI_Reduce(MPI_IN_PLACE, non_bonded_energy, p->n_types, MPI_SOMA_SCALAR,
               MPI_SUM, 0, p->info_MPI.SOMA_comm_sim);
  else
    MPI_Reduce(non_bonded_energy, NULL, p->n_types, MPI_SOMA_SCALAR, MPI_SUM, 0,
               p->info_MPI.SOMA_comm_sim);
#endif // ENABLE_MPI
}

void calc_bonded_energy(const struct Phase *const p,
                        soma_scalar_t *const bonded_energy) {
  ...
  for (unsigned int poly = 0; poly < p->n_polymers; poly++) {
    ...
    for (unsigned int mono = 0; mono < N; mono++) {
      ...
      const int start = get_bondlist_offset(
          p->poly_arch[p->poly_type_offset[type] + mono + 1]);
      ...
      if (start > 0) {
        ...
        for (int i = start; end == 0; i++) {
          ...
          if (offset > 0) // Select each bond only once, i<j
          {
            ...
            const int mic_flag = p->args.bond_minimum_image_convention_flag;
            const soma_scalar_t dx = calc_bond_length(
                beads[mono].x, beads[mono_j].x, p->Lx, mic_flag);
            const soma_scalar_t dy = calc_bond_length(
                beads[mono].y, beads[mono_j].y, p->Ly, mic_flag);
            const soma_scalar_t dz = calc_bond_length(
                beads[mono].z, beads[mono_j].z, p->Lz, mic_flag);
...
#if (ENABLE_MPI == 1)
  if (p->info_MPI.sim_rank == 0)
    MPI_Reduce(MPI_IN_PLACE, bonded_energy, NUMBER_SOMA_BOND_TYPES,
               MPI_SOMA_SCALAR, MPI_SUM, 0, p->info_MPI.SOMA_comm_sim);
  else
    MPI_Reduce(bonded_energy, NULL, NUMBER_SOMA_BOND_TYPES, MPI_SOMA_SCALAR,
               MPI_SUM, 0, p->info_MPI.SOMA_comm_sim);
#endif // ENABLE_MPI
}

int extent_ana_by_field(const soma_scalar_t *const data, const uint64_t n_data,
                        const char *const name, const hid_t file_id) {
  herr_t status;
  ...
  if (ndims != 2) {
    fprintf(stderr,
            "ERROR: %s:%d not the correct number of dimensions to extent the "
            "data set for %s.\n",
  ...
  if (dims[0] > 0) {
    status = H5Sselect_hyperslab(filespace, H5S_SELECT_SET, dims_offset, NULL,
                                 dims_memspace, NULL);
    HDF5_ERROR_CHECK(status);
  } else
    memspace = H5S_ALL;

  status = H5Dwrite(dset, H5T_SOMA_NATIVE_SCALAR, memspace, filespace,
                    H5P_DEFAULT, data);
  HDF5_ERROR_CHECK(status);
...
int extent_density_field_old(const struct Phase *const p,
                             const void *const field_pointer,
  ...
  const unsigned int buffer_size =
      (p->nx / p->args.N_domains_arg) * p->ny * p->nz;
  ...
  if (p->info_MPI.sim_rank == 0) {
    ...
    if (ndims != 5) {
      fprintf(stderr,
              "ERROR: %s:%d not the correct number of dimensions to extent "
              "density field %s.\n",
              __FILE__, __LINE__, name);
      return -1;
    }
    ...
    for (unsigned int type = 0; type < p->n_types; type++) {
      ...
      memcpy(ptr,
             field_pointer + ghost_buffer_size * data_size +
                 p->n_cells_local * type * data_size,
             buffer_size * data_size);
      for (int i = 0; i < p->args.N_domains_arg; i++) {
        filespace = H5Dget_space(dset);
        HDF5_ERROR_CHECK(filespace);

        status = H5Sselect_hyperslab(filespace, H5S_SELECT_SET, dims_offset,
                                     NULL, dims_memspace, NULL);
        HDF5_ERROR_CHECK(status);
...
#if (ENABLE_MPI == 1) // Safe, because i+1 < p->args.N_domains_arg = 1 for
                      // non-mpi simulation
          MPI_Recv(ptr, buffer_size, mpi_type, rank,
                   (i + 1) + type * p->args.N_domains_arg,
    ...
    status = H5Dclose(dset);
    HDF5_ERROR_CHECK(status);
  } else if (p->info_MPI.domain_rank ==
             0) { // Send data to sim rank to write the data
    for (unsigned int type = 0; type < p->n_types; type++) {
#if (ENABLE_MPI == 1)
      MPI_Send(field_pointer + ghost_buffer_size * data_size +
                   type * p->n_cells_local * data_size,
               buffer_size, mpi_type, 0,
               my_domain + type * p->args.N_domains_arg,
               p->info_MPI.SOMA_comm_sim);
#endif // ENABLE_MPI
    }
  }
...
int extent_density_field(const struct Phase *const p, void *const field_pointer,
  ...
  if (p->info_MPI.sim_rank == 0) {
    ...
    void *full_array = field_pointer; // If only a single MPI rank is used, no
                                      // gathering is needed
#if (ENABLE_MPI == 1)
    if (p->info_MPI.sim_size > 1) {
      full_array =
          malloc(p->n_types * p->nx * p->ny * p->nz * sizeof(data_size));
      if (full_array == NULL) {
        fprintf(stderr, "Malloc ERROR %s:%d\n", __FILE__, __LINE__);
        return -1;
      }
      for (unsigned int type = 0; type < p->n_types; type++) {
        MPI_Gather(field_pointer + ghost_buffer_size * data_size +
                       type * p->n_cells_local * data_size,
                   buffer_size, mpi_type,
                   full_array + type * p->nx * p->ny * p->nz * data_size,
                   buffer_size, mpi_type, 0,
    ...
    HDF5_ERROR_CHECK(status);
    status =
        H5Dwrite(dset, hdf5_type, memspace, filespace, plist_id, full_array);
    HDF5_ERROR_CHECK(status);
    ...
    for (unsigned int type = 0; type < p->n_types; type++) {
      MPI_Gather(field_pointer + ghost_buffer_size * data_size +
                     type * p->n_cells_local * data_size,
                 buffer_size, mpi_type, NULL, 0, mpi_type, 0,
                 p->ana_info.inter_domain_communicator);
    }
#endif // ENABLE_MPI
  }
...
int analytics(struct Phase *const p) {
  ...
  if (p->ana_info.delta_mc_Re != 0 && p->time % p->ana_info.delta_mc_Re == 0) {
    ...
    soma_scalar_t *const Re = (soma_scalar_t *const)malloc(
        4 * p->n_poly_type * sizeof(soma_scalar_t));
    if (Re == NULL) {
  ...
  // dvar calculation
  if (p->ana_info.delta_mc_density_var != 0 &&
      p->time % p->ana_info.delta_mc_density_var == 0) {
    update_self_phase(p, 0);
  ...
  if (p->ana_info.delta_mc_Rg != 0 && p->time % p->ana_info.delta_mc_Rg == 0) {
    update_self_phase(p, 0);
    soma_scalar_t *const Rg = (soma_scalar_t *const)malloc(
        4 * p->n_poly_type * sizeof(soma_scalar_t));
    if (Rg == NULL) {
      fprintf(stderr, "ERROR: Malloc %s:%d \n", __FILE__, __LINE__);
      return -2;
  ...
  // Bond anisotropy
  if (p->ana_info.delta_mc_b_anisotropy != 0 &&
      p->time % p->ana_info.delta_mc_b_anisotropy == 0) {
    update_self_phase(p, 0);
    soma_scalar_t *const a = (soma_scalar_t *const)malloc(
        6 * p->n_poly_type * sizeof(soma_scalar_t));
    if (a == NULL) {
      fprintf(stderr, "ERROR: Malloc %s:%d \n", __FILE__, __LINE__);
      return -2;
    }
    calc_anisotropy(p, a);
    if (p->info_MPI.sim_rank == 0)
      extent_ana_by_field(a, 6 * p->n_poly_type, "/bond_anisotropy",
                          p->ana_info.file_id);
    written = true;
    free(a);
  }
  ...
  // Acceptance ratio
  if (p->ana_info.delta_mc_acc_ratio != 0 &&
      p->time % p->ana_info.delta_mc_acc_ratio == 0) {
  ...
  // MSD
  if (p->ana_info.delta_mc_MSD != 0 &&
      p->time % p->ana_info.delta_mc_MSD == 0) {
    soma_scalar_t *const msd = (soma_scalar_t *const)malloc(
        8 * p->n_poly_type * sizeof(soma_scalar_t));
    if (msd == NULL) {
      fprintf(stderr, "ERROR: Malloc %s:%d \n", __FILE__, __LINE__);
      return -2;
    }
...
#pragma acc update self(p->fields_unified [0:p->n_cells_local * p->n_types])
  ...
  if (p->ana_info.delta_mc_bonded_energy != 0 &&
    ...
    soma_scalar_t *const b_energy = (soma_scalar_t *const)malloc(
        NUMBER_SOMA_BOND_TYPES * sizeof(soma_scalar_t));
    if (b_energy == NULL) {
      fprintf(stderr, "ERROR: Malloc %s:%d \n", __FILE__, __LINE__);
      return -2;
    }
    ...
    if (p->info_MPI.sim_rank == 0)
      extent_ana_by_field(b_energy, NUMBER_SOMA_BOND_TYPES, "/bonded_energy",
                          p->ana_info.file_id);
    written = true;
    free(b_energy);
  }
  ...
  if (p->ana_info.delta_mc_density_field != 0 &&
      p->time % p->ana_info.delta_mc_density_field == 0) {
    // if we run on one core+gpu only we avoid the stepwise field transfer, so
    // for writing fields we need to update the local field
    if (p->info_MPI.sim_size == 1) {
...
#pragma acc update self(p->fields_unified [0:p->n_cells_local * p->n_types])
    ...
    extent_density_field(p, p->fields_unified, "/density_field",
                         H5T_NATIVE_UINT16, MPI_UINT16_T, sizeof(uint16_t));
    written = true;
  }

  // umbrella_field
  if (p->ana_info.delta_mc_umbrella_field != 0 &&
      p->time % p->ana_info.delta_mc_umbrella_field == 0) {
    if (p->info_MPI.sim_size == 1) {
#pragma acc update self(p->fields_unified [0:p->n_cells * p->n_types])
    }
    extent_density_field(p, p->umbrella_field, "/umbrella_field",
                         H5T_SOMA_NATIVE_SCALAR, MPI_SOMA_SCALAR,
  ...
  if (p->ana_info.delta_mc_dynamical_structure != 0 &&
      p->time % p->ana_info.delta_mc_dynamical_structure == 0) {
    // update_self_phase(p,0); //update not needed, because the calculation is
    // on the device
    soma_scalar_t *const dynamical_structure_factor =
        (soma_scalar_t *const)malloc(p->ana_info.q_size_dynamical *
                                     p->n_poly_type * p->n_types * p->n_types *
                                     sizeof(soma_scalar_t));
    ...
    if (p->info_MPI.sim_rank == 0) {
      extent_structure(p, dynamical_structure_factor,
                       "/dynamical_structure_factor", p->ana_info.file_id,
                       sf_type);
    }
    written = true;
  ...
  if (p->ana_info.delta_mc_static_structure != 0 &&
      p->time % p->ana_info.delta_mc_static_structure == 0) {
    // update_self_phase(p,0); //update not needed, because the calculation is
    // on the device
    soma_scalar_t *const static_structure_factor = (soma_scalar_t *const)malloc(
        p->ana_info.q_size_static * p->n_poly_type * p->n_types * p->n_types *
        sizeof(soma_scalar_t));
    ...
    if (p->info_MPI.sim_rank == 0) {
      extent_structure(p, static_structure_factor, "/static_structure_factor",
                       p->ana_info.file_id, sf_type);
    }
    written = true;
  ...
  if (p->ana_info.delta_mc_dump != 0 &&
      p->time % p->ana_info.delta_mc_dump == 0) {
    update_self_phase(p, 0);
    const unsigned int len = 1024;
    char filename[1024]; // len
    memset(filename, '\0', len * sizeof(char));
    const unsigned int check =
        sprintf(filename, "%s_t%u.h5", p->ana_info.coord_filename, p->time);
    if (check >= len)
  ...
  if (written && p->info_MPI.sim_rank ==
                     0) // If something has been written to hdf5 flush it.
  {
...
int calc_structure(const struct Phase *p, soma_scalar_t *const result,
                   const enum structure_factor_type sf_type) {
  ...
  uint64_t *const counter =
      (uint64_t *const)calloc(p->n_poly_type, sizeof(uint64_t));
  if (counter == NULL) {
    fprintf(stderr, "MALLOC ERROR: %s:%d\n", __FILE__, __LINE__);
    return -1;
  }
  memset(counter, 0, p->n_poly_type * sizeof(uint64_t));
  memset(result, 0,
         q_size * p->n_poly_type * p->n_types * p->n_types *
             sizeof(soma_scalar_t));
  unsigned int n_random_q = p->args.n_random_q_arg;
  soma_scalar_t *const result_tmp = (soma_scalar_t *const)malloc(
      n_random_q * p->n_polymers * result_tmp_size * sizeof(soma_scalar_t));
  if (result_tmp == NULL) {
    fprintf(stderr, "MALLOC ERROR: %s:%d\n", __FILE__, __LINE__);
    return -1;
  }
  for (unsigned int index = 0;
       index < n_random_q * p->n_polymers * result_tmp_size; index++) {
    result_tmp[index] = 0;
  }
  soma_scalar_t *const tmp = (soma_scalar_t *const)malloc(
      n_random_q * p->n_polymers * q_size * p->n_types * p->n_types *
      sizeof(soma_scalar_t));
  if (tmp == NULL) {
    fprintf(stderr, "MALLOC ERROR: %s:%d\n", __FILE__, __LINE__);
    return -1;
...
#pragma acc enter data copyin(                                                 \
    result_tmp [0:n_random_q * p->n_polymers * result_tmp_size],               \
    q_array [0:q_size])
#pragma acc enter data copyin(                                                 \
    tmp [0:n_random_q * p->n_polymers * q_size * p->n_types * p->n_types])     \
    async
...
#pragma acc loop // be careful, seq?
    for (unsigned int index_random_q = 0; index_random_q < n_random_q;
         index_random_q++) {
...
#pragma acc loop seq
      for (unsigned int mono = 0; mono < poly_length; mono++) {
        const unsigned int particle_type = get_particle_type(
            p->poly_arch[p->poly_type_offset[poly_type] + 1 + mono]);
...
#pragma acc loop seq
        for (unsigned int index_q = 0; index_q < q_size; index_q++) {
          soma_scalar_t qr =
              q_array[index_q] * (unit_q_x * x + unit_q_y * y + unit_q_z * z);

          switch (sf_type) {
          case STATIC_STRUCTURE_FACTOR:
            result_tmp[index_random_q * p->n_polymers * q_size * p->n_types *
                           2 +
                       poly * q_size * p->n_types * 2 +
                       index_q * p->n_types * 2 + particle_type * 2 + 0] +=
                cos(qr);
            result_tmp[index_random_q * p->n_polymers * q_size * p->n_types *
                           2 +
                       poly * q_size * p->n_types * 2 +
                       index_q * p->n_types * 2 + particle_type * 2 + 1] +=
                sin(qr);
            break;
          case DYNAMICAL_STRUCTURE_FACTOR:;
            ...
            soma_scalar_t qr_msd =
                q_array[index_q] *
                (unit_q_x * x_0 + unit_q_y * y_0 + unit_q_z * z_0);
            result_tmp[index_random_q * p->n_polymers * q_size * p->n_types *
                           4 +
                       poly * q_size * p->n_types * 4 +
                       index_q * p->n_types * 4 + particle_type * 4 + 0] +=
                cos(qr);
            result_tmp[index_random_q * p->n_polymers * q_size * p->n_types *
                           4 +
                       poly * q_size * p->n_types * 4 +
                       index_q * p->n_types * 4 + particle_type * 4 + 1] +=
                sin(qr);
            result_tmp[index_random_q * p->n_polymers * q_size * p->n_types *
                           4 +
                       poly * q_size * p->n_types * 4 +
                       index_q * p->n_types * 4 + particle_type * 4 + 2] +=
                cos(qr_msd);
            result_tmp[index_random_q * p->n_polymers * q_size * p->n_types *
                           4 +
                       poly * q_size * p->n_types * 4 +
                       index_q * p->n_types * 4 + particle_type * 4 + 3] +=
                sin(qr_msd);
...
#pragma acc loop seq
    for (unsigned int index_random_q = 0; index_random_q < n_random_q;
         index_random_q++) {
#pragma acc loop seq
      for (unsigned int particle_type_i = 0; particle_type_i < p->n_types;
           particle_type_i++) {
#pragma acc loop seq
        for (unsigned int particle_type_j = 0; particle_type_j < p->n_types;
             particle_type_j++) {
#pragma acc loop seq
          for (unsigned int index_q = 0; index_q < q_size; index_q++) {
            switch (sf_type) {
...
#if (ENABLE_MPI == 1)
  MPI_Allreduce(MPI_IN_PLACE, result,
                q_size * p->n_poly_type * p->n_types * p->n_types,
                MPI_SOMA_SCALAR, MPI_SUM, p->info_MPI.SOMA_comm_sim);
#endif
  free(result_tmp);
  free(tmp);
  free(counter);
...
int extent_structure(const struct Phase *p, const soma_scalar_t *const data,
                     const char *const name, const hid_t file_id,
                     const enum structure_factor_type sf_type) {
  unsigned int size;
