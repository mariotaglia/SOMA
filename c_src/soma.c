...
int main(int argc, char *argv[]) {
  ...
  const int error =
      MPI_Comm_dup(MPI_COMM_WORLD, &(p->info_MPI.SOMA_comm_world));
  ...
  const int error2 =
      MPI_Comm_dup(p->info_MPI.SOMA_comm_world, &(p->info_MPI.SOMA_comm_sim));
  ...
  if (post_args < 0) {
    fprintf(stderr,
            "Post processing the arguments on rank %d failed. Exiting.\n",
            p->info_MPI.world_rank);
    return -3;
  }
  ...
  MPI_ERROR_CHECK(init_domain_chains_status,
                  "Sending chains for domain decomposition failed.");
#endif // ENABLE_MPI
  ...
  for (unsigned int i = 0; i < N_steps; i++) {
    ...
    if (p->pc.deltaMC > 0 &&
        i % p->pc.deltaMC == (unsigned int)p->pc.deltaMC - 1)
      convert_polytypes(p);

#if (ENABLE_MPI == 1)
    if (p->args.load_balance_arg > 0 &&
        i % p->args.load_balance_arg ==
            (unsigned int)p->args.load_balance_arg - 1)
    ...
    if (!p->args.no_sync_signal_flag) {
      // Sync all mpi cores
      MPI_Allreduce(MPI_IN_PLACE, &stop_iteration, 1, MPI_INT, MPI_SUM,
                    p->info_MPI.SOMA_comm_world);
    }
#endif // ENABLE_MPI
    ...
    if (stop_iteration) {
      if (p->info_MPI.world_rank == 0)
        fprintf(
            stdout,
            "Environment to stop iteration at time %d catched by rank %d.\n",
