...
typedef struct MERSENNE_TWISTER_STATE {
  ...
  uint32_t internal_state[MTMAX_num_int_state]; //!<\brief Internal state of the
                                                //!< Mersenne-Twister
...
typedef struct TT800STATE {
  ...
  uint32_t
      internal_state[TT_num_int_state]; //!<\brief Internal state of the TT800
} TT800STATE;

//! Struct to hold the global arrays of heany data for alternative PRNGs
//!
//! Each Phase has one of these structs. And in case an alternative RNG is used,
//! this struct contains the memory for the states. If the memory is not used
...
//!
//!\param mt_rng is the struct which contains the internal state of the random
//! number generator \return uint32
...
//! Function initializes the internal state of thr reduced Mersenne-Twister
//! TT800 with the PCG32 \param rng  struct which contains all information about
//! PCG32 \param mt_rng  is the struct which contains the internal state of the
//! random number generator \return int
