...
int init_ana(struct Phase *const p, const char *const filename, const char *const coord_filename)
{
    ...
    p->ana_info.delta_mc_npos_field = 0;
    p->ana_info.delta_mc_nneg_field = 0;
...
/// npos field
    if (p->ana_info.delta_mc_npos_field > 0)
        {
            //Exception safety:
            const unsigned int tmd_delta_mc_string = p->ana_info.delta_mc_npos_field;
            p->ana_info.delta_mc_npos_field = 0;
#pragma acc update device(p->ana_info)
            const hsize_t three = 3;
            const hsize_t one = 1;

            hid_t dataset = H5Dopen2(p->ana_info.file_id, "/npos_field", H5P_DEFAULT);
            status = dataset;
            HDF5_ERROR_CHECK(status);
            ...
            if (nxyz_exists > 0)
                {               //Nxyz attr exists
                    hid_t nxyz_attr = H5Aopen_by_name(dataset, "/npos_field", "nxyz", H5P_DEFAULT, H5P_DEFAULT);
                    unsigned int nxyz_tmp[3];
                    status = H5Aread(nxyz_attr, H5T_NATIVE_UINT, nxyz_tmp);
                    HDF5_ERROR_CHECK(status);
                    if ((nxyz_tmp[0] != p->nx) || (nxyz_tmp[1] != p->ny) || (nxyz_tmp[2] != p->nz))
                        {
                            fprintf(stderr, "WARNING: existing nxyz attr of npos_field"
                                    "does not match with the system data. No density field output possible. (Try with fresh ana.h5 file.).\n");
                            return 0;
                        }
                    status = H5Aclose(nxyz_attr);
            ...
            if (ntypes_exists > 0)
                {               //n_types attr exists
                    hid_t ntypes_attr = H5Aopen_by_name(dataset, "/npos_field", "ntypes", H5P_DEFAULT, H5P_DEFAULT);
                    unsigned int ntypes_tmp;
                    status = H5Aread(ntypes_attr, H5T_NATIVE_UINT, &ntypes_tmp);
                    HDF5_ERROR_CHECK(status);
                    if ((ntypes_tmp != 1))
                        {
                            fprintf(stderr, "Error: ntypes for npos field should be 1."
                                    "Currently, it does not match with the system data. No density field output possible. \n");
                            return 0;
                        }
                    status = H5Aclose(ntypes_attr);
            ...
            if (lxyz_exists > 0)
                {               //lxyz attr exists
                    hid_t lxyz_attr = H5Aopen_by_name(dataset, "/npos_field", "lxyz", H5P_DEFAULT, H5P_DEFAULT);
                    soma_scalar_t lxyz_tmp[3];
                    status = H5Aread(lxyz_attr, H5T_SOMA_NATIVE_SCALAR, lxyz_tmp);
                    HDF5_ERROR_CHECK(status);
                    if ((lxyz_tmp[0] != p->Lx) || (lxyz_tmp[1] != p->Ly) || (lxyz_tmp[2] != p->Lz))
                        {
                            fprintf(stderr, "WARNING: existing lxyz attr of npos_field"
                                    "does not match with the system data. No dumping possible.\n");
                            return 0;
                        }
                    status = H5Aclose(lxyz_attr);
            ...
            if (ndims != 5)
                {
                    fprintf(stderr, "ERROR: %s:%d not the correct number of dimensions for a npos_field.\n",
                            __FILE__, __LINE__);
                    return -3;
                }
            hsize_t dims[5];    //ndims
            ...
            if (dims[1] != 1 || dims[2] != p->nx || dims[3] != p->ny || dims[4] != p->nz)
                {
                    fprintf(stderr,
                            "Error: The npos_field dimensions do not match! No npos_field output possible. \n");
                    return 0;
                }
            status = H5Dclose(dataset);
            HDF5_ERROR_CHECK(status);
            ...
            //If everything was successful set the ana period again.
            p->ana_info.delta_mc_npos_field = tmd_delta_mc_string;
        }

/// nneg field
    if (p->ana_info.delta_mc_nneg_field > 0)
        {
            //Exception safety:
            const unsigned int tmd_delta_mc_string = p->ana_info.delta_mc_nneg_field;
            p->ana_info.delta_mc_nneg_field = 0;
#pragma acc update device(p->ana_info)
            const hsize_t three = 3;
            const hsize_t one = 1;

            hid_t dataset = H5Dopen2(p->ana_info.file_id, "/nneg_field", H5P_DEFAULT);
            status = dataset;
            HDF5_ERROR_CHECK(status);
            ...
            if (nxyz_exists > 0)
                {               //Nxyz attr exists
                    hid_t nxyz_attr = H5Aopen_by_name(dataset, "/nneg_field", "nxyz", H5P_DEFAULT, H5P_DEFAULT);
                    unsigned int nxyz_tmp[3];
                    status = H5Aread(nxyz_attr, H5T_NATIVE_UINT, nxyz_tmp);
                    HDF5_ERROR_CHECK(status);
                    if ((nxyz_tmp[0] != p->nx) || (nxyz_tmp[1] != p->ny) || (nxyz_tmp[2] != p->nz))
                        {
                            fprintf(stderr, "WARNING: existing nxyz attr of nneg_field"
                                    "does not match with the system data. No density field output possible. (Try with fresh ana.h5 file.).\n");
                            return 0;
                        }
                    status = H5Aclose(nxyz_attr);
            ...
            if (ntypes_exists > 0)
                {               //n_types attr exists
                    hid_t ntypes_attr = H5Aopen_by_name(dataset, "/nneg_field", "ntypes", H5P_DEFAULT, H5P_DEFAULT);
                    unsigned int ntypes_tmp;
                    status = H5Aread(ntypes_attr, H5T_NATIVE_UINT, &ntypes_tmp);
                    HDF5_ERROR_CHECK(status);
                    if ((ntypes_tmp != 1))
                        {
                            fprintf(stderr, "Error: ntypes for nneg field should be 1."
                                    "Currently, it does not match with the system data. No density field output possible. \n");
                            return 0;
                        }
                    status = H5Aclose(ntypes_attr);
            ...
            if (lxyz_exists > 0)
                {               //lxyz attr exists
                    hid_t lxyz_attr = H5Aopen_by_name(dataset, "/nneg_field", "lxyz", H5P_DEFAULT, H5P_DEFAULT);
                    soma_scalar_t lxyz_tmp[3];
                    status = H5Aread(lxyz_attr, H5T_SOMA_NATIVE_SCALAR, lxyz_tmp);
                    HDF5_ERROR_CHECK(status);
                    if ((lxyz_tmp[0] != p->Lx) || (lxyz_tmp[1] != p->Ly) || (lxyz_tmp[2] != p->Lz))
                        {
                            fprintf(stderr, "WARNING: existing lxyz attr of nneg_field"
                                    "does not match with the system data. No dumping possible.\n");
                            return 0;
                        }
                    status = H5Aclose(lxyz_attr);
            ...
            if (ndims != 5)
                {
                    fprintf(stderr, "ERROR: %s:%d not the correct number of dimensions for a nneg_field.\n",
                            __FILE__, __LINE__);
                    return -3;
                }
            hsize_t dims[5];    //ndims
            ...
            if (dims[1] != 1 || dims[2] != p->nx || dims[3] != p->ny || dims[4] != p->nz)
                {
                    fprintf(stderr,
                            "Error: The nneg_field dimensions do not match! No nneg_field output possible. \n");
                    return 0;
                }
            status = H5Dclose(dataset);
            HDF5_ERROR_CHECK(status);
            ...
            //If everything was successful set the ana period again.
            p->ana_info.delta_mc_nneg_field = tmd_delta_mc_string;
        }
