...
int init_ana(struct Phase *const p, const char *const filename,
             const char *const coord_filename) {
...
#define SOMA_NUM_OBS 13
  ...
  for (unsigned int i = 0; i < SOMA_NUM_OBS; i++) {
    ...
    if (status < 0)
      fprintf(
          stderr,
          "WARNING %s:%d ana element %s not existing (old file version?).\n",
          __FILE__, __LINE__, names[i]);
    else {
      hid_t attr = H5Aopen_by_name(dataset, names[i], "DeltaMC", H5P_DEFAULT,
                                   H5P_DEFAULT);
      status = attr;
      if (status < 0)
        fprintf(
            stderr,
            "WARNING %s:%d deltamc for %s not existing (old file version?).\n",
            __FILE__, __LINE__, names[i]);
      else {
  ...
  if (p->ana_info.delta_mc_dump > 0) {
    if (coord_filename != NULL) {
      ...
      if (p->ana_info.coord_filename == NULL) {
        fprintf(stderr,
                "ERROR: %s %d malloc. So no automatic dumping possible.\n",
                __FILE__, __LINE__);
        p->ana_info.delta_mc_dump = 0;
      }
      ...
      p->ana_info.delta_mc_dump = 0;
      fprintf(stderr,
              "WARNING: %s %d no coord_file specified. So no automatic dumping "
              "possible.\n",
              __FILE__, __LINE__);
    }
#pragma acc update device(p->ana_info)
...
#pragma acc update device(p->ana_info)
    ...
    hid_t dataset =
        H5Dopen2(p->ana_info.file_id, "/density_field", H5P_DEFAULT);
    status = dataset;
    HDF5_ERROR_CHECK(status);
    ...
    if (nxyz_exists > 0) { // Nxyz attr exists
      hid_t nxyz_attr = H5Aopen_by_name(dataset, "/density_field", "nxyz",
                                        H5P_DEFAULT, H5P_DEFAULT);
      unsigned int nxyz_tmp[3];
      status = H5Aread(nxyz_attr, H5T_NATIVE_UINT, nxyz_tmp);
      HDF5_ERROR_CHECK(status);
      if ((nxyz_tmp[0] != p->nx) || (nxyz_tmp[1] != p->ny) ||
          (nxyz_tmp[2] != p->nz)) {
        fprintf(stderr, "WARNING: existing nxyz attr of density_field"
                        "does not match with the system data. No density field "
                        "output possible. (Try with fresh ana.h5 file.).\n");
        return 0;
      }
      ...
      hid_t nxyz_attr = H5Acreate2(dataset, "nxyz", H5T_STD_U32LE,
                                   nxyz_dataspace, H5P_DEFAULT, H5P_DEFAULT);
    ...
    if (ntypes_exists > 0) { // n_types attr exists
      hid_t ntypes_attr = H5Aopen_by_name(dataset, "/density_field", "ntypes",
                                          H5P_DEFAULT, H5P_DEFAULT);
      ...
      hid_t ntypes_attr =
          H5Acreate2(dataset, "ntypes", H5T_STD_U32LE, ntypes_dataspace,
                     H5P_DEFAULT, H5P_DEFAULT);
    ...
    if (lxyz_exists > 0) { // lxyz attr exists
      hid_t lxyz_attr = H5Aopen_by_name(dataset, "/density_field", "lxyz",
                                        H5P_DEFAULT, H5P_DEFAULT);
      soma_scalar_t lxyz_tmp[3];
      status = H5Aread(lxyz_attr, H5T_SOMA_NATIVE_SCALAR, lxyz_tmp);
      HDF5_ERROR_CHECK(status);
      if ((lxyz_tmp[0] != p->Lx) || (lxyz_tmp[1] != p->Ly) ||
          (lxyz_tmp[2] != p->Lz)) {
        fprintf(stderr,
                "WARNING: existing lxyz attr of density_field"
      ...
      hid_t lxyz_attr = H5Acreate2(dataset, "lxyz", H5T_SOMA_FILE_SCALAR,
                                   lxyz_dataspace, H5P_DEFAULT, H5P_DEFAULT);
    ...
    if (typescale_exists > 0) { // type_scale attr exists
      hid_t typescale_attr = H5Aopen_by_name(
          dataset, "/density_field", "typescale", H5P_DEFAULT, H5P_DEFAULT);
      ...
      hid_t typescale_attr =
          H5Acreate2(dataset, "typescale", H5T_SOMA_FILE_SCALAR,
                     typescale_dataspace, H5P_DEFAULT, H5P_DEFAULT);
      HDF5_ERROR_CHECK(typescale_attr);
      status = H5Awrite(typescale_attr, H5T_SOMA_NATIVE_SCALAR,
    ...
    if (ndims != 5) {
      fprintf(stderr,
              "ERROR: %s:%d not the correct number of dimensions for a density "
              "field.\n",
              __FILE__, __LINE__);
      return -3;
    }
    ...
    HDF5_ERROR_CHECK(status);
    if (dims[1] != p->n_types || dims[2] != p->nx || dims[3] != p->ny ||
        dims[4] != p->nz) {
      fprintf(stderr,
              "Error: The density field dimensions do not match! No density "
              "field output possible. (Try with fresh ana.h5 file.)\n");
      return 0;
    }
  ...
  if (p->ana_info.delta_mc_umbrella_field > 0) {
    // Exception safety:
    const unsigned int tmd_delta_mc_string =
        p->ana_info.delta_mc_umbrella_field;
    p->ana_info.delta_mc_umbrella_field = 0;
#pragma acc update device(p->ana_info)
    const hsize_t three = 3;
    const hsize_t one = 1;
    ...
    hid_t dataset =
        H5Dopen2(p->ana_info.file_id, "/umbrella_field", H5P_DEFAULT);
    status = dataset;
    HDF5_ERROR_CHECK(status);
    ...
    htri_t nxyz_exists = H5Aexists(dataset, "nxyz");
    if (nxyz_exists > 0) { // Nxyz attr exists
      hid_t nxyz_attr = H5Aopen_by_name(dataset, "/umbrella_field", "nxyz",
                                        H5P_DEFAULT, H5P_DEFAULT);
      unsigned int nxyz_tmp[3];
      status = H5Aread(nxyz_attr, H5T_NATIVE_UINT, nxyz_tmp);
      HDF5_ERROR_CHECK(status);
    ...
    htri_t ntypes_exists = H5Aexists(dataset, "ntypes");
    if (ntypes_exists > 0) { // n_types attr exists
      hid_t ntypes_attr = H5Aopen_by_name(dataset, "/umbrella_field", "ntypes",
                                          H5P_DEFAULT, H5P_DEFAULT);
      unsigned int ntypes_tmp;
      status = H5Aread(ntypes_attr, H5T_NATIVE_UINT, &ntypes_tmp);
      HDF5_ERROR_CHECK(status);
    ...
    htri_t lxyz_exists = H5Aexists(dataset, "lxyz");
    if (lxyz_exists > 0) { // lxyz attr exists
      hid_t lxyz_attr = H5Aopen_by_name(dataset, "/umbrella_field", "lxyz",
                                        H5P_DEFAULT, H5P_DEFAULT);
      soma_scalar_t lxyz_tmp[3];
      status = H5Aread(lxyz_attr, H5T_SOMA_NATIVE_SCALAR, lxyz_tmp);
      HDF5_ERROR_CHECK(status);
      if ((lxyz_tmp[0] != p->Lx) || (lxyz_tmp[1] != p->Ly) ||
          (lxyz_tmp[2] != p->Lz)) {
        fprintf(stderr,
                "WARNING: existing lxyz attr of umbrella_field"
                "does not match with the system data. No dumping possible.\n");
        return 0;
      }
      status = H5Aclose(lxyz_attr);
    ...
    if (typescale_exists > 0) { // type_scale attr exists
      hid_t typescale_attr = H5Aopen_by_name(
          dataset, "/umbrella_field", "typescale", H5P_DEFAULT, H5P_DEFAULT);
      soma_scalar_t *const typescale_tmp =
          (soma_scalar_t *const)malloc(p->n_types * sizeof(soma_scalar_t));
      if (typescale_tmp == NULL) {
        fprintf(stderr, "ERROR: Malloc %s:%d \n", __FILE__, __LINE__);
      ...
      if (!match) {
        fprintf(stderr,
                "Error: existing typescale attr of umbrella_field"
                "does not match with the system data. No umbrella_field output "
                "possible. (Try with fresh ana.h5 file.)\n");
        return 0;
      }
      status = H5Aclose(typescale_attr);
      HDF5_ERROR_CHECK(status);
    ...
    if (ndims != 5) {
      fprintf(stderr,
              "ERROR: %s:%d not the correct number of dimensions for a "
              "umbrella_field.\n",
              __FILE__, __LINE__);
      return -3;
    }
    ...
    if (dims[1] != p->n_types || dims[2] != p->nx || dims[3] != p->ny ||
      ...
      fprintf(
          stderr,
          "Error: The umbrella_field dimensions do not match! No "
          "umbrella_field output possible. (Try with fresh ana.h5 file.)\n");
      return 0;
    }
    status = H5Dclose(dataset);
    HDF5_ERROR_CHECK(status);
  ...
  if (p->ana_info.delta_mc_Re > 0) {
    ...
    p->ana_info.delta_mc_Re = 0;
    p->end_mono =
        (unsigned int *)malloc(2 * p->n_poly_type * sizeof(unsigned int));
    if (p->end_mono == NULL) {
    ...
    if (end_mono_exists > 0) { // end_mono attr exists
      hid_t end_mono_attr =
          H5Aopen_by_name(dataset, "/Re", "end_mono", H5P_DEFAULT, H5P_DEFAULT);
      ...
      if (dims[0] != p->n_poly_type) {
        fprintf(stderr,
                "ERROR: %s:%d end_mono for %d polymers, but expected is %d.\n",
                __FILE__, __LINE__, (int)dims[0], p->n_poly_type);
        return -1;
      }
      if (dims[1] != 2) {
        fprintf(stderr,
                "ERROR: %s:%d end_mono for %d elements for start/end, but "
                "expected is 2.\n",
      ...
      for (unsigned int poly_type = 0; poly_type < p->n_poly_type;
           poly_type++) {
        const unsigned int N = p->poly_arch[p->poly_type_offset[poly_type]];
        if (p->end_mono[poly_type * 2 + 0] >= N ||
            p->end_mono[poly_type * 2 + 1] >= N) {
          fprintf(
              stderr,
              "ERROR: %s:%d EndMono contains invalid configuration. %d %d %d",
      ...
      fprintf(stderr,
              "ERROR: analysis of Re requires an end_mono attribute.\n");
      return -2;
    }
    status = H5Dclose(dataset);
    HDF5_ERROR_CHECK(status);
  ...
  if (p->ana_info.delta_mc_non_bonded_energy > 0) { // Sanitize input
    const unsigned int tmp = p->ana_info.delta_mc_non_bonded_energy;
    p->ana_info.delta_mc_non_bonded_energy = 0;
    hid_t dataset =
        H5Dopen2(p->ana_info.file_id, "/non_bonded_energy", H5P_DEFAULT);
    hid_t status = dataset;
    HDF5_ERROR_CHECK(status);
    hid_t d_space = H5Dget_space(dataset);
    const unsigned int ndims = H5Sget_simple_extent_ndims(d_space);
  ...
  if (p->ana_info.delta_mc_bonded_energy > 0) { // Sanitize input
    const unsigned int tmp = p->ana_info.delta_mc_bonded_energy;
    p->ana_info.delta_mc_bonded_energy = 0;
    hid_t dataset =
        H5Dopen2(p->ana_info.file_id, "/bonded_energy", H5P_DEFAULT);
    hid_t status = dataset;
    HDF5_ERROR_CHECK(status);
    hid_t d_space = H5Dget_space(dataset);
    const unsigned int ndims = H5Sget_simple_extent_ndims(d_space);
    ...
    if (dims[1] != NUMBER_SOMA_BOND_TYPES) {
      fprintf(stderr, "Error: bonded energy dataspace is incorrect. (Might be "
                      "an incompatible version problem.)\n");
      return -2;
    }
    status = H5Dclose(dataset);
    HDF5_ERROR_CHECK(status);
  ...
  if (p->ana_info.delta_mc_dynamical_structure > 0) {
    ...
    hid_t dataset = H5Dopen2(p->ana_info.file_id, "/dynamical_structure_factor",
                             H5P_DEFAULT);
    hid_t status = dataset;
    HDF5_ERROR_CHECK(status);

    htri_t q_size_exists = H5Aexists(dataset, "q_size");
    if (q_size_exists > 0) { // q_size attr exists
      hid_t q_size_attr =
          H5Aopen_by_name(dataset, "/dynamical_structure_factor", "q_size",
                          H5P_DEFAULT, H5P_DEFAULT);
      ...
      status = H5Sget_simple_extent_dims(d_space, dims, NULL);
      HDF5_ERROR_CHECK(status);
      status = H5Aread(q_size_attr, H5T_NATIVE_UINT,
                       &(p->ana_info.q_size_dynamical));
      HDF5_ERROR_CHECK(status);
      status = H5Aclose(q_size_attr);
      HDF5_ERROR_CHECK(status);
    }

    p->ana_info.q_dynamical = (soma_scalar_t *)malloc(
        p->ana_info.q_size_dynamical * sizeof(soma_scalar_t));
    ...
    if (q_exists > 0) { // q attr exists
      hid_t q_attr = H5Aopen_by_name(dataset, "/dynamical_structure_factor",
                                     "q", H5P_DEFAULT, H5P_DEFAULT);
  ...
  if (p->ana_info.delta_mc_static_structure > 0) {
    ...
    hid_t dataset =
        H5Dopen2(p->ana_info.file_id, "/static_structure_factor", H5P_DEFAULT);
    hid_t status = dataset;
    HDF5_ERROR_CHECK(status);

    htri_t q_size_exists = H5Aexists(dataset, "q_size");
    if (q_size_exists > 0) { // q_size attr exists
      hid_t q_size_attr = H5Aopen_by_name(dataset, "/static_structure_factor",
                                          "q_size", H5P_DEFAULT, H5P_DEFAULT);
      hid_t d_space = H5Aget_space(q_size_attr);
      HDF5_ERROR_CHECK(d_space);
      hsize_t dims[1]; // ndims
      status = H5Sget_simple_extent_dims(d_space, dims, NULL);
      HDF5_ERROR_CHECK(status);
      status =
          H5Aread(q_size_attr, H5T_NATIVE_UINT, &(p->ana_info.q_size_static));
      HDF5_ERROR_CHECK(status);
      status = H5Aclose(q_size_attr);
      HDF5_ERROR_CHECK(status);
    }

    p->ana_info.q_static = (soma_scalar_t *)malloc(p->ana_info.q_size_static *
    ...
    htri_t q_exists = H5Aexists(dataset, "q");
    if (q_exists > 0) { // q attr exists
      hid_t q_attr = H5Aopen_by_name(dataset, "/static_structure_factor", "q",
                                     H5P_DEFAULT, H5P_DEFAULT);
      hid_t d_space = H5Aget_space(q_attr);
      HDF5_ERROR_CHECK(d_space);
      const unsigned int ndims = H5Sget_simple_extent_ndims(d_space);
...
#if (ENABLE_MPI == 1)
  if (MPI_Comm_split(p->info_MPI.SOMA_comm_sim, p->info_MPI.domain_rank,
                     p->info_MPI.sim_rank,
