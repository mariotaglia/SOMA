...
int set_neighbour(const unsigned int jbead, const Monomer *const neigh,
  ...
  do {
    ...
    move_allowed = !possible_move_area51(p, neigh->x, neigh->y, neigh->z, dx.x,
                                         dx.y, dx.z, false);
  } while (move_allowed);
...
int generate_new_beads(struct Phase *const p) {
  if (fabs(p->harmonic_normb_variable_scale) < 1e-5) {
    fprintf(
        stderr,
        "WARNING: p->harmonic_normb_variable_scale < 1e-5, this may result in "
        "unreasonable generated position or even causes divisions by 0.\n");
  }
  const unsigned int my_domain = p->info_MPI.sim_rank / p->info_MPI.domain_size;
  const soma_scalar_t domain_offset =
      my_domain * (p->Lx / p->args.N_domains_arg);
  for (uint64_t i = 0; i < p->n_polymers; i++) {
    ...
    while (free_index >= 0) {
      ...
      do {
        ...
        domain_counter += 1;
        idx = coord_to_index(
            p, x, y, z); // > p->n_cells_local if position is out of the domain
      } while (p->area51 != NULL &&
               (idx >= p->n_cells_local || p->area51[idx] == 1));
      ...
      const int old_bead_bondlist_offset = get_bondlist_offset(
      ...
      if (old_bead_bondlist_offset != -1) {
        ...
        do {
          do {
            if (end != 0) { // if all the bonds are already set, go back
              chain_index--;
              if (chain_index < 0)
                break;
              bondlist = get_bondlist_offset(
                  p->poly_arch[p->poly_type_offset[poly->type] +
                               chain[chain_index] + 1]);
          ...
          chain_index++;
          set_neighbour(new_bead, &(beads[old_bead]), bond_type, already_set,
                        poly, p);
          already_set[new_bead] = true;
          total_set++;
          bondlist = get_bondlist_offset(
              p->poly_arch[p->poly_type_offset[poly->type] + new_bead + 1]);
  ...
  memcpy(p->ph.msd_beads.ptr, p->ph.beads.ptr,
         p->ph.beads.used * p->ph.beads.typelength);
