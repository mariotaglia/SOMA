...
static inline void coord_to_cell_coordinate(const struct Phase *p,
                                            const soma_scalar_t rx,
                                            const soma_scalar_t ry,
                                            const soma_scalar_t rz, int *x,
                                            int *y, int *z);
inline void coord_to_cell_coordinate(const struct Phase *p,
                                     const soma_scalar_t rx,
                                     const soma_scalar_t ry,
                                     const soma_scalar_t rz, int *x, int *y,
                                     int *z) {
...
*/
static inline uint64_t cell_coordinate_to_index(const struct Phase *p,
                                                const int x, const int y,
                                                const int z);
#pragma acc routine(cell_coordinate_to_index) seq
inline uint64_t cell_coordinate_to_index(const struct Phase *p, const int x,
...
static inline uint64_t coord_to_index(const struct Phase *const p,
                                      const soma_scalar_t rx,
                                      const soma_scalar_t ry,
...
static inline uint64_t cell_to_index_unified(const struct Phase *const p,
                                             const uint64_t cell,
                                             const unsigned int rtype);
inline uint64_t cell_to_index_unified(const struct Phase *const p,
                                      const uint64_t cell,
                                      const unsigned int rtype) {
...
/*! \brief Calculate an index for any given position and type and the density
 field / external field of p. This function should ALWAYS be called to get an
 index of a field. Issues:
...
static inline uint64_t coord_to_index_unified(const struct Phase *const p,
                                              const soma_scalar_t rx,
                                              const soma_scalar_t ry,
                                              const soma_scalar_t rz,
                                              unsigned int rtype);
inline uint64_t coord_to_index_unified(const struct Phase *p,
                                       const soma_scalar_t rx,
                                       const soma_scalar_t ry,
                                       const soma_scalar_t rz,
                                       const unsigned int rtype) {
...
/*! Update the density fields \f$ \rho \f$ of the system.
 The field update fist sets all entries to zero. Then performs a loop over all
 polymers eg. monomers. The field itself is chosen according to the monomer
 type. Be shure that there is no additional type set otherwise the adressing
 fails. \param p Phase configuration to update the  densityfields. \return
 Errorcode \note this call requires MPI collectives so call it on every
 MPI-core.
 */
...
//! \f[\frac{H}{k_BT } = \frac{\rho_0}{N_{ref}}\int_V D(\{r\})
//! \sum_i^{n_{types}}\frac{k_i}{2} \left(\phi_i(r)-s_i(r)\right)^2 +
//! \sum_i^{n_{types}} \phi_i(r) f_i(r) + \frac{\kappa N_{ref}}{2} (\sum_{i}
//! ^{n_{types}} \phi_i(r) - 1)^2 - \sum_{i<j}^{n_{types}}
//! \frac{\chi_{i,j}N_{ref}}{4} (\phi_i(r) - \phi_j(r))^2 \f] \f[
//! \frac{\omega_k(c)}{k_BT} = \frac{N}{k_BT \rho_0 \Delta L^3}\frac{\partial
//! H(c)}{k_BT \partial \phi_k} = \sum_i^{n_{types}}
//! k_i\left(\phi_i(c)-s_i(c)\right) + f_k(c) + \kappa (\sum_i^{n_{types}}
//! \phi_i(c)-1) - \sum_{i< k}^{n_{types}} \frac{\chi_{i,k}}{2}
//! (\phi_k(c)-\phi_i(c))   \f] \note In case of the use of the density_weights
//! every \f$\phi_i\f$ is multiplied with its weight \f$w_i\f$. For this
//! Hamiltonian weights differing from 1 are not recommended -- use SCMF1
//! instead.
...
//! \f[\frac{H}{k_BT } = \frac{\rho_0}{N_{ref}}\int_V D(\{r\})
//! \sum_i^{n_{types}} \frac{k_i}{2} \left(\phi_i(r)-s_i(r)\right)^2+
//! \sum_i^{n_{types}} \phi_i(r) f_i(r) + \frac{\kappa N_{ref}}{2}
//! \left(\sum_{i} ^{n_{types}} \phi_i(r) - 1\right)^2 + \sum_{i< j}^{n_{types}}
//! \chi_{i,j}N_{ref} \phi_i(r) \phi_j(r) \f] \f[ \omega_k(c) =
//! \frac{N_{ref}}{k_BT\rho_0 \Delta L^3}\frac{\partial H(c)}{\partial \phi_k} =
//! \sum_i^{n_{types}} k_i\left(\phi_i(c)-s_i(c)\right)+ f_k(c) + \kappa
//! \left(\sum_i^{n_{types}} \phi_i(c)-1\right) + \sum_{i< k}^{n_{types}}
//! \chi_{i,k} \phi_i(c)   \f] \note In case of the use of the density_weights
//! every \f$\phi_i\f$ is multiplied with its weight \f$w_i\f$.
