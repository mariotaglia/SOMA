...
/*! Helper function to write area51 to the config HDF5 file.
    ...
    \return Errorcode
*/
int write_area51_hdf5(const struct Phase *const p, const hid_t file_id,
                      const hid_t plist_id) {
  const unsigned int my_domain = p->info_MPI.sim_rank / p->info_MPI.domain_size;
  const unsigned int ghost_buffer_size =
      p->args.domain_buffer_arg * p->ny * p->nz;

  const hsize_t hsize_dataspace[3] = {p->nx, p->ny, p->nz};
  hid_t dataspace = H5Screate_simple(3, hsize_dataspace, NULL);
  const hsize_t hsize_memspace[3] = {p->nx / p->args.N_domains_arg, p->ny,
                                     p->nz};
  hid_t memspace = H5Screate_simple(3, hsize_memspace, NULL);

  hid_t dataset = H5Dcreate2(file_id, "/area51", H5T_STD_U8LE, dataspace,
                             H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);

  dataspace = H5Dget_space(dataset);
  const hsize_t offset[3] = {my_domain * (p->nx / p->args.N_domains_arg), 0, 0};
  H5Sselect_hyperslab(dataspace, H5S_SELECT_SET, offset, NULL, hsize_memspace,
                      NULL);
...
#endif // ENABLE_MPI
  int status;
  if ((status = H5Dwrite(dataset, H5T_NATIVE_UINT8, memspace, dataspace,
                         plist_id, p->area51 + ghost_buffer_size)) < 0) {
...
/*! Helper function to write scalar fields (external_field, umbrella_field) to
   the config HDF5 file. \private \param p Phase describing the system \param
   file_id File identifier of open HDF5 file. \param plist_id Access properties
   to use. \param field Pointer to the scalar field \param name Name of the
   field. \return Errorcode
*/
int write_field_hdf5(struct Phase *const p, const hid_t file_id,
                     const hid_t plist_id, const soma_scalar_t *const field,
                     const char *name) {
  const unsigned int my_domain = p->info_MPI.sim_rank / p->info_MPI.domain_size;
  const unsigned int ghost_buffer_size =
      p->args.domain_buffer_arg * p->ny * p->nz;
  ...
  const hsize_t hsize_memspace[4] = {1, p->nx / p->args.N_domains_arg, p->ny,
                                     p->nz};
  ...
  for (unsigned int type = 0; type < p->n_types; type++) {
    ...
    const hsize_t offset[4] = {
        type, my_domain * (p->nx / p->args.N_domains_arg), 0, 0};

    H5Sselect_hyperslab(dataspace, H5S_SELECT_SET, offset, NULL, hsize_memspace,
                        NULL);
    const soma_scalar_t *const ptr =
        field + ghost_buffer_size + type * p->n_cells_local;
    if ((status = H5Dwrite(dataset, H5T_SOMA_NATIVE_SCALAR, memspace, dataspace,
                           plist_id, ptr)) < 0) {
      fprintf(stderr, "ERROR: core: %d HDF5-error %s:%d code %d\n",
              p->info_MPI.world_rank, __FILE__, __LINE__, status);
      return status;
...
\return Errorcode
*/
int write_poly_type(const struct Phase *const p, hid_t file_id,
                    const hid_t plist_id) {
  hid_t status;
  ...
  MPI_Scan((uint64_t *)&(p->n_polymers), &n_polymer_offset, 1, MPI_UINT64_T,
  ...
  H5Sselect_hyperslab(poly_type_dataspace, H5S_SELECT_SET,
                      &(hsize_polymers_offset), NULL, &(hsize_polymers), NULL);

  if ((status = H5Dwrite(poly_type_dataset, H5T_NATIVE_UINT, poly_type_memspace,
...
\return Errorcode
*/
int write_poly_tag(const struct Phase *const p, hid_t file_id,
                   const hid_t plist_id) {
  hid_t status;
  // Determine the offset, for each process.
  uint64_t n_polymer_offset = 0;
  ...
  uint64_t *const poly_tag =
      (uint64_t *const)malloc(p->n_polymers_storage * sizeof(uint64_t));
  if (poly_tag == NULL) {
    fprintf(stderr, "ERROR: Malloc %s:%d\n", __FILE__, __LINE__);
    return -1;
  }
  ...
  hsize_t hsize_polymers_global = p->n_polymers_global;
  hid_t poly_tag_dataspace =
      H5Screate_simple(1, &(hsize_polymers_global), NULL);
  hsize_t hsize_polymers = p->n_polymers;
  hid_t poly_tag_memspace = H5Screate_simple(1, &(hsize_polymers), NULL);

  hid_t poly_tag_dataset =
      H5Dcreate2(file_id, "/poly_tag", H5T_STD_U64LE, poly_tag_dataspace,
                 H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);

  poly_tag_dataspace = H5Dget_space(poly_tag_dataset);
  hsize_t hsize_polymers_offset = n_polymer_offset;
  H5Sselect_hyperslab(poly_tag_dataspace, H5S_SELECT_SET,
                      &(hsize_polymers_offset), NULL, &(hsize_polymers), NULL);

  if ((status = H5Dwrite(poly_tag_dataset, H5T_NATIVE_UINT64, poly_tag_memspace,
                         poly_tag_dataspace, plist_id, poly_tag)) < 0) {
    fprintf(stderr, "ERROR: core: %d HDF5-error %s:%d code %d\n",
            p->info_MPI.world_rank, __FILE__, __LINE__, (int)status);
    return status;
  }
...
/*! Helper function to write the heavy beads dataset to file. With multiple
ranks, MPI-IO is used.
...
\return Errorcode
*/
int write_beads(const struct Phase *const p, hid_t file_id,
                const hid_t plist_id) {
  hid_t status;
  ...
  hid_t beads_dataset =
      H5Dcreate2(file_id, "/beads", H5T_SOMA_FILE_SCALAR, beads_dataspace,
                 H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);

  uint64_t bead_offset = 0;
#if (ENABLE_MPI == 1)
  // Cast for MPI_Scan, since some openmpi impl. need a non-const. version.
  MPI_Scan((uint64_t *)&(p->num_all_beads_local), &bead_offset, 1, MPI_UINT64_T,
           MPI_SUM, p->info_MPI.SOMA_comm_sim);
  ...
  H5Sselect_hyperslab(beads_dataspace, H5S_SELECT_SET, hsize_beads_offset, NULL,
                      hsize_beads_memspace, NULL);

  soma_scalar_t *const monomer_data = (soma_scalar_t *const)malloc(
      p->num_all_beads_local * 3 * sizeof(soma_scalar_t));
  ...
  for (uint64_t i = 0; i < p->n_polymers; i++) {
    ...
    const unsigned int N =
        p->poly_arch[p->poly_type_offset[p->polymers[i].type]];
  ...
  if ((status = H5Dwrite(beads_dataset, H5T_SOMA_NATIVE_SCALAR, beads_memspace,
...
int write_config_hdf5(struct Phase *const p, const char *filename) {
  ...
  hid_t parameter_group =
      H5Gcreate2(file_id, "/parameter", H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);

  const hsize_t one = 1;
  // Write the version number of the fileformat (hard coded) change if the
  // version changes. A version change make the file incompatible to read/write
  const unsigned int version = 1;
  status = write_hdf5(1, &one, file_id, "/version", H5T_STD_U32LE,
                      H5T_NATIVE_UINT, plist_id, &version);
  HDF5_ERROR_CHECK2(status, "/version");

  // Write number of polymers
  status = write_hdf5(1, &one, file_id, "/parameter/n_polymers", H5T_STD_U64LE,
                      H5T_NATIVE_UINT64, plist_id, &(p->n_polymers_global));
  HDF5_ERROR_CHECK2(status, "/parameter/n_polymers");

  status =
      write_hdf5(1, &one, file_id, "/parameter/reference_Nbeads", H5T_STD_U32LE,
                 H5T_NATIVE_UINT, plist_id, &(p->reference_Nbeads));
  HDF5_ERROR_CHECK2(status, "/parameter/reference_Nbeads");

  status = write_hdf5(1, &one, file_id, "/parameter/n_types", H5T_STD_U32LE,
                      H5T_NATIVE_UINT, plist_id, &(p->n_types));
  HDF5_ERROR_CHECK2(status, "/parameter/n_types");

  // Number of types
  hsize_t xn_dim[2] = {p->n_types, p->n_types};
  status = write_hdf5(2, xn_dim, file_id, "/parameter/xn", H5T_SOMA_FILE_SCALAR,
                      H5T_SOMA_NATIVE_SCALAR, plist_id, p->xn);
  HDF5_ERROR_CHECK2(status, "/parameter/xn");

  // A data
  hsize_t n_types_size = p->n_types;
  status =
      write_hdf5(1, &n_types_size, file_id, "/parameter/A",
                 H5T_SOMA_FILE_SCALAR, H5T_SOMA_NATIVE_SCALAR, plist_id, p->A);
  ...
  for (unsigned int i = 0; i < p->n_types; i++)
    p->field_scaling_type[i] /=
        (p->n_accessible_cells / (soma_scalar_t)p->num_all_beads);
  status = write_hdf5(1, &n_types_size, file_id, "/parameter/density_weights",
                      H5T_SOMA_FILE_SCALAR, H5T_SOMA_NATIVE_SCALAR, plist_id,
  ...
  // time
  status = write_hdf5(1, &one, file_id, "/parameter/time", H5T_STD_U32LE,
                      H5T_NATIVE_UINT, plist_id, &(p->time));
  HDF5_ERROR_CHECK2(status, "/parameter/time");

  status = write_hdf5(1, &one, file_id, "/parameter/hamiltonian", H5T_STD_I32LE,
                      H5T_NATIVE_INT, plist_id, &(p->hamiltonian));
  HDF5_ERROR_CHECK2(status, "parameter/hamiltonian");

  n_types_size = p->n_types;
  status = write_hdf5(1, &n_types_size, file_id, "/parameter/k_umbrella",
                      H5T_SOMA_FILE_SCALAR, H5T_SOMA_NATIVE_SCALAR, plist_id,
                      p->k_umbrella);
  ...
  status = write_hdf5(1, &three, file_id, "/parameter/nxyz", H5T_STD_U32LE,
                      H5T_NATIVE_UINT, plist_id, nxyz);
  HDF5_ERROR_CHECK2(status, "/parameter/nxyz");

  // Lx Ly Lz
  soma_scalar_t lxyz[3] = {p->Lx, p->Ly, p->Lz};
  status =
      write_hdf5(1, &three, file_id, "/parameter/lxyz", H5T_SOMA_FILE_SCALAR,
                 H5T_SOMA_NATIVE_SCALAR, plist_id, lxyz);
  HDF5_ERROR_CHECK2(status, "/parameter/lxyz");

  // p->harmonic_normb_variable_scale
  status =
      write_hdf5(1, &one, file_id, "/parameter/harmonic_normb_variable_scale",
                 H5T_IEEE_F64LE, H5T_SOMA_NATIVE_SCALAR, plist_id,
                 &(p->harmonic_normb_variable_scale));
  ...
  status =
      write_hdf5(1, &n_poly_type, file_id, "/parameter/poly_type_offset",
                 H5T_STD_I32LE, H5T_NATIVE_INT, plist_id, p->poly_type_offset);
  HDF5_ERROR_CHECK2(status, "poly_type_offset");
  // poly_arch_length
  status =
      write_hdf5(1, &one, file_id, "/parameter/poly_arch_length", H5T_STD_U32LE,
  ...
  status = write_hdf5(1, &arch_length, file_id, "/parameter/poly_arch",
                      H5T_STD_I32LE, H5T_NATIVE_INT, plist_id, p->poly_arch);
  HDF5_ERROR_CHECK2(status, "poly_arch");

  if (p->cm_a != NULL) {
    status = write_hdf5(1, &n_poly_type, file_id, "/parameter/cm_a",
                        H5T_SOMA_FILE_SCALAR, H5T_SOMA_NATIVE_SCALAR, plist_id,
                        p->cm_a);
  ...
  if (p->external_field_unified) {
    hid_t status = write_field_hdf5(
        p, file_id, plist_id, p->external_field_unified, "/external_field");
    if (status != 0) {
  ...
  if (p->umbrella_field) {
    hid_t status = write_field_hdf5(p, file_id, plist_id, p->umbrella_field,
                                    "/umbrella_field");
    if (status != 0) {
...
/*! Helper function to read area51 from the config HDF5 file.
    ...
    \return Errorcode
*/
int read_area51_hdf5(struct Phase *const p, const hid_t file_id,
                     const hid_t plist_id) {
  const unsigned int my_domain = p->info_MPI.sim_rank / p->info_MPI.domain_size;
  const unsigned int ghost_buffer_size =
      p->args.domain_buffer_arg * p->ny * p->nz;
  ...
  p->area51 = (uint8_t *)malloc(
      (p->nx / p->args.N_domains_arg + 2 * p->args.domain_buffer_arg) * p->ny *
      p->nz * sizeof(uint8_t));
  if (p->area51 == NULL) {
    fprintf(stderr, "ERROR: Malloc %s:%d\n", __FILE__, __LINE__);
    return -1;
  ...
  int status;
  if ((status = H5Dread(dataset, H5T_STD_U8LE, memspace, dataspace, plist_id,
                        p->area51 + ghost_buffer_size)) < 0) {
    fprintf(stderr, "ERROR: core: %d HDF5-error %s:%d code %d\n",
            p->info_MPI.world_rank, __FILE__, __LINE__, (int)status);
    return status;
  }
  ...
  const int left_neigh_rank =
      (((my_domain - 1) + p->args.N_domains_arg) % p->args.N_domains_arg) *
  ...
  MPI_Isend(ptr, ghost_buffer_size, MPI_UINT8_T, left_neigh_rank, 0,
            p->info_MPI.SOMA_comm_sim, req + 0);
  ptr = p->area51 + ((p->nx / p->args.N_domains_arg) * p->ny * p->nz);
  MPI_Isend(ptr, ghost_buffer_size, MPI_UINT8_T, right_neigh_rank, 1,
            p->info_MPI.SOMA_comm_sim, req + 1);

  ptr = p->area51;
  MPI_Irecv(ptr, ghost_buffer_size, MPI_UINT8_T, left_neigh_rank, 1,
            p->info_MPI.SOMA_comm_sim, req + 2);
  ptr = p->area51 + ((p->nx / p->args.N_domains_arg) * p->ny * p->nz) +
        ghost_buffer_size;
  MPI_Irecv(ptr, ghost_buffer_size, MPI_UINT8_T, right_neigh_rank, 0,
...
/*! Helper function to read scalar fields (external_field, umbrella_field) from
   the config HDF5 file. \private \param p Phase describing the system \param
   file_id File identifier of open HDF5 file. \param plist_id Access properties
   to use. \param field Pointer to the scalar field \param name Name of the
   field. \return Errorcode
*/
int read_field_hdf5(const struct Phase *const p, const hid_t file_id,
                    const hid_t plist_id, soma_scalar_t **field,
                    const char *name) {
  const unsigned int my_domain = p->info_MPI.sim_rank / p->info_MPI.domain_size;
  const unsigned int ghost_buffer_size =
      p->args.domain_buffer_arg * p->ny * p->nz;
  ...
  const uint64_t n_cells_local =
      (p->nx / p->args.N_domains_arg + 2 * p->args.domain_buffer_arg) * p->ny *
      p->nz;
  *field = (soma_scalar_t *)malloc(n_cells_local * p->n_types *
  ...
  for (unsigned int type = 0; type < p->n_types; type++) {
    ...
    if ((status = H5Dread(dataset, H5T_SOMA_NATIVE_SCALAR, memspace, dataspace,
                          plist_id, ptr)) < 0) {
      fprintf(stderr, "ERROR: core: %d HDF5-error %s:%d code %d\n",
              p->info_MPI.world_rank, __FILE__, __LINE__, (int)status);
      return status;
    ...
    MPI_Isend(ptr, ghost_buffer_size, MPI_SOMA_SCALAR, left_neigh_rank,
              0 + 2 * type, p->info_MPI.SOMA_comm_sim, req + 0);
    ptr = *field + ((p->nx / p->args.N_domains_arg) * p->ny * p->nz) +
          type * n_cells_local;
    MPI_Isend(ptr, ghost_buffer_size, MPI_SOMA_SCALAR, right_neigh_rank,
              1 + 2 * type, p->info_MPI.SOMA_comm_sim, req + 1);

    ptr = *field + type * n_cells_local;
    MPI_Irecv(ptr, ghost_buffer_size, MPI_SOMA_SCALAR, left_neigh_rank,
              1 + 2 * type, p->info_MPI.SOMA_comm_sim, req + 2);
    ptr = *field + ((p->nx / p->args.N_domains_arg) * p->ny * p->nz) +
          ghost_buffer_size + type * n_cells_local;
    MPI_Irecv(ptr, ghost_buffer_size, MPI_SOMA_SCALAR, right_neigh_rank,
              0 + 2 * type, p->info_MPI.SOMA_comm_sim, req + 3);
...
/*! Helper function to read the beads information from the input files of
version 1.
...
\return Errorcode.
*/
int read_beads1(struct Phase *const p, const hid_t file_id,
                const hid_t plist_id) {
  hid_t status;
  // temporary load poly type to calculate polymer distribution
  unsigned int *poly_type_tmp =
      (unsigned int *)malloc(p->n_polymers_global * sizeof(unsigned int));
  if (poly_type_tmp == NULL) {
    fprintf(stderr, "ERROR: Malloc %s:%d\n", __FILE__, __LINE__);
    return -1;
  }
  status = read_hdf5(file_id, "/poly_type", H5T_NATIVE_UINT, plist_id,
                     poly_type_tmp);
  ...
  for (int sim_rank = 0; sim_rank < p->info_MPI.sim_rank + 1; sim_rank++) {
    my_num_poly = 0;
    my_num_beads = 0;
    while (my_num_beads < av_beads_per_rank &&
           poly_offset + my_num_poly < p->n_polymers_global) {
      const unsigned int N =
          p->poly_arch
              [p->poly_type_offset[poly_type_tmp[poly_offset + my_num_poly]]];
      my_num_beads += N;
      my_num_poly += 1;
    }
  ...
  /* printf("DEBUG %s:%d (remove) sim rank: %d bead_offset: %d poly_offset: %d
   * num_beads: %d num_poly: %d\n", __FILE__, */
  /*        __LINE__, p->info_MPI.sim_rank, (int)bead_offset, (int)poly_offset,
   * (int)my_num_beads, (int)my_num_poly); */
  ...
  if (H5Lexists(file_id, "/poly_tag", H5P_DEFAULT) > 0) {
    ...
    if ((status =
             H5Dread(poly_tag_dataset, H5T_NATIVE_UINT64, poly_tag_memspace,
                     poly_tag_dataspace, plist_id, poly_tag)) < 0) {
      fprintf(stderr, "ERROR: core: %d HDF5-error %s:%d code %d\n",
              p->info_MPI.world_rank, __FILE__, __LINE__, (int)status);
      return status;
  ...
  for (uint64_t i = 0; i < p->n_polymers; i++) {
    ...
    if (p->polymers[i].bead_offset == UINT64_MAX) {
      fprintf(stderr, "ERROR: invalid memory alloc %s:%d rank %d, n_poly %lu\n",
              __FILE__, __LINE__, p->info_MPI.world_rank, p->n_polymers);
      return -1;
    }
    p->polymers[i].msd_bead_offset =
        get_new_soma_memory_offset(&(p->ph.msd_beads), N);
  ...
  if (H5Lexists(file_id, "/beads", H5P_DEFAULT) > 0) {
    ...
    hsize_t hsize_beads_offset[2] = {bead_offset, 0};
    H5Sselect_hyperslab(beads_dataspace, H5S_SELECT_SET, hsize_beads_offset,
                        NULL, hsize_beads_memspace, NULL);

    if ((status = H5Dread(beads_dataset, H5T_SOMA_NATIVE_SCALAR, beads_memspace,
                          beads_dataspace, plist_id, monomer_data)) < 0) {
      fprintf(stderr, "ERROR: core: %d HDF5-error %s:%d code %d\n",
              p->info_MPI.world_rank, __FILE__, __LINE__, (int)status);
      return status;
...
/*! General helper function to read the beads information from the input file if
available.

This function wraps the different actual reading functions and selects the
appropriate one according to the file version. \private \param p Phase struct
which is going to be initialized. \param file_id opened HDF5 file handle to read
from. \param plist_id HDF5 property list to read with. \return Errorcode.
*/
int read_beads(struct Phase *p, const hid_t file_id, const hid_t plist_id) {
  ...
  if (H5Lexists(file_id, "/version", H5P_DEFAULT) > 0) {
    status =
        read_hdf5(file_id, "/version", H5T_NATIVE_UINT, plist_id, &version);
    HDF5_ERROR_CHECK2(status, "version");
  }
  switch (version) {
  case 0:
...
int read_config_hdf5(struct Phase *const p, const char *filename) {
  ...
  status = read_hdf5(file_id, "/parameter/n_polymers", H5T_NATIVE_UINT64,
                     plist_id, &(p->n_polymers_global));
  HDF5_ERROR_CHECK2(status, "/parameter/n_polymers");

  status = read_hdf5(file_id, "/parameter/reference_Nbeads", H5T_NATIVE_UINT,
                     plist_id, &(p->reference_Nbeads));
  HDF5_ERROR_CHECK2(status, "/parameter/reference_Nbeads");

  status = read_hdf5(file_id, "/parameter/n_types", H5T_NATIVE_UINT, plist_id,
                     &(p->n_types));
  HDF5_ERROR_CHECK2(status, "/parameter/n_types");

  p->xn = (soma_scalar_t *const)malloc(p->n_types * p->n_types *
                                       sizeof(soma_scalar_t));
  if (p->xn == NULL) {
    fprintf(stderr, "ERROR: Malloc %s:%d\n", __FILE__, __LINE__);
    return -1;
  ...
  status = read_hdf5(file_id, "/parameter/xn", H5T_SOMA_NATIVE_SCALAR, plist_id,
                     p->xn);
  HDF5_ERROR_CHECK2(status, "/parameter/xn");
  ...
  // read p->A
  status = read_hdf5(file_id, "/parameter/A", H5T_SOMA_NATIVE_SCALAR, plist_id,
                     p->A);
  HDF5_ERROR_CHECK2(status, "/parameter/A");

  // read p->time
  status = read_hdf5(file_id, "/parameter/time", H5T_NATIVE_UINT, plist_id,
                     &(p->time));
  HDF5_ERROR_CHECK2(status, "/parameter/time");
  ...
  if (H5Lexists(file_id, "/parameter/k_umbrella", H5P_DEFAULT) > 0) {
    status = read_hdf5(file_id, "/parameter/k_umbrella", H5T_SOMA_NATIVE_SCALAR,
  ...
  unsigned int nxyz[3];
  status =
      read_hdf5(file_id, "/parameter/nxyz", H5T_NATIVE_UINT, plist_id, nxyz);
  HDF5_ERROR_CHECK2(status, "/parameter/nxyz");
  p->nx = nxyz[0];
  p->ny = nxyz[1];
  p->nz = nxyz[2];
  ...
  status = read_hdf5(file_id, "/parameter/lxyz", H5T_SOMA_NATIVE_SCALAR,
                     plist_id, lxyz);
  HDF5_ERROR_CHECK2(status, "/parameter/lxyz");
  p->Lx = lxyz[0];
  p->Ly = lxyz[1];
  ...
  // Number of polymer type
  status = read_hdf5(file_id, "/parameter/n_poly_type", H5T_NATIVE_UINT,
                     plist_id, &(p->n_poly_type));
  ...
  status = read_hdf5(file_id, "/parameter/poly_type_offset", H5T_NATIVE_INT,
                     plist_id, p->poly_type_offset);
  HDF5_ERROR_CHECK2(status, "poly_type_offset");
  // poly_arch_length
  status = read_hdf5(file_id, "/parameter/poly_arch_length", H5T_NATIVE_UINT,
                     plist_id, &(p->poly_arch_length));
  HDF5_ERROR_CHECK2(status, "poly_arch_length");
  // poly_arch
  ...
  status = read_hdf5(file_id, "/parameter/poly_arch", H5T_NATIVE_INT32,
                     plist_id, p->poly_arch);
  HDF5_ERROR_CHECK2(status, "poly_arch");
  ...
  if (H5Lexists(file_id, "/parameter/harmonic_normb_variable_scale",
                H5P_DEFAULT) > 0) {
    status = read_hdf5(file_id, "/parameter/harmonic_normb_variable_scale",
                       H5T_SOMA_NATIVE_SCALAR, plist_id,
                       &(p->harmonic_normb_variable_scale));
    HDF5_ERROR_CHECK2(status, "/parameter/harmonic_normb_variable_scale");
  } else {
    if (p->info_MPI.sim_rank == 0) {
      if (get_number_bond_type(p, HARMONICVARIABLESCALE) != 0)
        fprintf(stderr,
                "WARNING: The poly_arch contains HARMONICVARIABLESCALE Bond, "
                "but no corresponding value is set. Using 1 instead.\n");
    }
  ...
  if (H5Lexists(file_id, "/external_field", H5P_DEFAULT) > 0) {
    ...
    if (time_exists <= 0) {
      p->serie_length = 1;
      p->cos_serie =
          (soma_scalar_t *)malloc(p->serie_length * sizeof(soma_scalar_t));
      p->sin_serie =
          (soma_scalar_t *)malloc(p->serie_length * sizeof(soma_scalar_t));
      p->sin_serie[0] = 0;
      p->cos_serie[0] = 1;
    ...
    if (time_exists > 0) { // time attr exists

      hid_t length_attr = H5Aopen_by_name(
          dataset, "/external_field", "serie_length", H5P_DEFAULT, H5P_DEFAULT);
      ...
      hid_t period_attr = H5Aopen_by_name(dataset, "/external_field", "period",
      ...
      hid_t cos_attr = H5Aopen_by_name(dataset, "/external_field", "cos",
                                       H5P_DEFAULT, H5P_DEFAULT);
      ...
      hid_t sin_attr = H5Aopen_by_name(dataset, "/external_field", "sin",
                                       H5P_DEFAULT, H5P_DEFAULT);
  ...
  if (H5Lexists(file_id, "/umbrella_field", H5P_DEFAULT) > 0) {
    hid_t status = read_field_hdf5(p, file_id, plist_id, &(p->umbrella_field),
                                   "/umbrella_field");
    if (status != 0) {
  ...
  if (status != 0) {
    fprintf(stderr,
            "ERROR: %s:%d unable to read polytype conversion information.\n",
            __FILE__, __LINE__);
    return status;
  }
  ...
  p->field_scaling_type =
      (soma_scalar_t *)malloc(p->n_types * sizeof(soma_scalar_t));
  ...
  if (H5Lexists(file_id, "/parameter/density_weights", H5P_DEFAULT) > 0) {
    hid_t status =
        read_hdf5(file_id, "/parameter/density_weights", H5T_SOMA_NATIVE_SCALAR,
                  plist_id, p->field_scaling_type);
    HDF5_ERROR_CHECK2(status, "/parameter/density_weights");
  } else
    for (unsigned int i = 0; i < p->n_types; i++)
...
int screen_output(struct Phase *const p, const unsigned int Nsteps) {
  ...
  const double second = now.tv_sec - p->start_clock.tv_sec +
                        (now.tv_usec - p->start_clock.tv_usec) * 1e-6;
  const unsigned int steps_done = p->time - p->start_time;
  const time_t end =
      p->start_clock.tv_sec + second * (Nsteps) / (soma_scalar_t)steps_done;
  ...
  if (p->args.screen_output_interval_arg > 0 &&
      now.tv_sec - last_print >= p->args.screen_output_interval_arg) {
    ...
    if (p->info_MPI.sim_rank == 0) {
      fprintf(stdout,
              "Rank %i:Running for %g [s] | TPS %g | steps-to-go: %u | ETA: %s",
              p->info_MPI.world_rank, second, tps, Nsteps - steps_done,
              ctime(&end));
      fflush(stdout);
    }
    last_print = now.tv_sec;
