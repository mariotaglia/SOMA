...
void sort_set_length(const unsigned int n_sets,
                     const unsigned int *const set_length,
                     unsigned int *const sort_array) {
  // Bubble sort is ok, because it is not time relevant and the data set is
  // small and the set is highly sorted.
...
//!
//! \private  Used only in the contex of independet set creation
bool try_particle_in_set(const unsigned int set_id, const unsigned int p_id,
                         const struct Phase *const p, const unsigned int N,
                         const unsigned int poly_type,
                         const unsigned int *const set_length,
                         const unsigned int *set_matrix) {
  bool neighbor_found = false;
  for (unsigned int iTmp = 0;
       iTmp < set_length[set_id] && neighbor_found == false; iTmp++) {
...
//!
//! \private  Used only in the contex of independet set creation
int balance_sets(const struct Phase *const p, const unsigned int N,
                 const unsigned int poly_type, unsigned int *const set_length,
                 unsigned int *const set_matrix, unsigned int recursion_level) {
  ...
  unsigned int *sort_array =
      (unsigned int *)malloc(n_sets * sizeof(unsigned int));
  if (sort_array == NULL) {
  ...
  do {
    ...
    solvable_unbalanced =
        false; // Set to true, if a element can be moved in the loop.
    ...
    for (unsigned int mono = 0; mono < set_length[sort_array[0]]; mono++) {
      ...
      for (new_set = n_sets - 1; new_set > 0; new_set--)
        // New set has to be smaller than the largest set, even if we remove one
        // element of the largest set.
        if (set_length[sort_array[new_set]] < set_length[sort_array[0]] - 1) {
          const bool suitable_set =
              try_particle_in_set(sort_array[new_set], move_id, p, N, poly_type,
                                  set_length, set_matrix);
          if (suitable_set)
            break;
        }

      if (new_set > 0) // Found an element to move and ****BREAK**** for loop,
                       // but do not stop while loop.
      {
        // reorganise the set_matrix of the largest set.
        for (unsigned int i = mono; i < set_length[sort_array[0]] - 1; i++)
          set_matrix[sort_array[0] * N + i] =
              set_matrix[sort_array[0] * N + i + 1];
        set_length[sort_array[0]] -= 1;

        // Insert the element in its new set.
        set_matrix[sort_array[new_set] * N + set_length[sort_array[new_set]]] =
            move_id;
  ...
  if (divergence > max_tolerated_divergence && n_sets < N) {
    set_length[sort_array[0]] -= 1;
    set_matrix[n_sets * N + 0] =
        set_matrix[sort_array[0] * N + set_length[sort_array[0]]];
    set_length[n_sets] += 1;
...
int generate_independet_sets(struct Phase *const p) {
  ...
  default:
    fprintf(stderr,
            "ERROR: Unknown independent set generation algorithm selected.\n");
    ret = -2;
  }
  return ret;
}
...
int independent_sets_simple(struct Phase *const p) {
  int ret = 0;
  struct IndependetSets *const sets =
      (struct IndependetSets *)malloc(p->n_poly_type * sizeof(IndependetSets));
  if (sets == NULL) {
    fprintf(stderr, "ERROR: malloc %s:%d\n", __FILE__, __LINE__);
    return -1;
  }
  ...
  for (unsigned int poly_type = 0; poly_type < p->n_poly_type; poly_type++) {
    ...
    unsigned int *const set_matrix =
        (unsigned int *)malloc(N * N * sizeof(unsigned int));
    if (set_matrix == NULL) {
      fprintf(stderr, "ERROR: malloc %s:%d\n", __FILE__, __LINE__);
      return -3;
    }
    ...
    for (unsigned int mono = 0; mono < N; mono++) {
      ...
      while (!set_found) {
        ...
        const bool suitable_set = try_particle_in_set(
            set_to_sort_in, mono, p, N, poly_type, set_length, set_matrix);
    ...
    sets[poly_type].sets =
        (unsigned int *)malloc(n_sets * max_set * sizeof(unsigned int));
    ...
    for (unsigned int iSet = 0; iSet < n_sets; iSet++) {
      ...
      for (unsigned int iMember = 0; iMember < set_length[iSet]; iMember++)
        sets[poly_type].sets[iSet * max_set + iMember] =
            set_matrix[iSet * N + iMember];
    }
...
int independent_set_fixed(struct Phase *const p) {
  ...
  for (unsigned int n_poly = 0; n_poly < p->n_poly_type; n_poly++) {
    ...
    if (ret != 0) {
      fprintf(stderr,
              "ERROR: Function independent_sets_one_polymer failed %s:%d\n",
              __FILE__, __LINE__);
      return ret;
    }
  }
...
int allo_init_memory_for_Polystates(struct Phase *const p) {
  init_soma_memory(&(p->ph.set_states), p->n_polymers * p->max_set_members,
                   sizeof(RNG_STATE));
  init_soma_memory(&(p->ph.set_permutation), p->n_polymers * p->max_n_sets,
                   sizeof(unsigned int));
  ...
  for (unsigned int i = 0; i < p->n_polymers; i++) {
    ...
    poly_tmp->set_states_offset =
        get_new_soma_memory_offset(&(p->ph.set_states), p->max_set_members);
    ...
    poly_tmp->set_permutation_offset =
        get_new_soma_memory_offset(&(p->ph.set_permutation), p->max_n_sets);
    ...
    const unsigned int seed =
        pcg32_random(&(poly_tmp->poly_state.default_state));
    for (unsigned int j = 0; j < p->max_set_members; j++) {
      ...
      case pseudo_random_number_generator_arg_PCG32:
        ...
        break;
      case pseudo_random_number_generator_arg_MT:
        state->alternative_rng_offset =
            get_new_soma_memory_offset(&(p->rh.mt), 1);
        if (state->alternative_rng_offset == UINT64_MAX) {
          fprintf(stderr,
                  "ERROR: invalid memory alloc %s:%d rank %d, n_poly %lu\n",
                  __FILE__, __LINE__, p->info_MPI.world_rank, p->n_polymers);
          return -1;
        }
        break;
      case pseudo_random_number_generator_arg_TT800:
        state->alternative_rng_offset =
            get_new_soma_memory_offset(&(p->rh.tt800), 1);
        if (state->alternative_rng_offset == UINT64_MAX) {
          fprintf(stderr,
                  "ERROR: invalid memory alloc %s:%d rank %d, n_poly %lu\n",
                  __FILE__, __LINE__, p->info_MPI.world_rank, p->n_polymers);
...
int independent_sets_one_polymer(struct IndependetSets **const set_tmp_pointer,
  ...
  unsigned int max_bond_number =
      0; // the maximal number of bonds of a monomer in the chain
  unsigned int max_bond = 0; // the monomer which has the maximal number of
                             // bonds
  ...
  unsigned int **bonds_total =
      (unsigned int **)malloc(sequence * sizeof(unsigned int *));
  if (bonds_total == NULL) {
    fprintf(stderr, "Malloc problem %s:%d\n", __FILE__, __LINE__);
    return -4;
  }
  ...
  for (unsigned int monomer_i = p->poly_type_offset[n_poly] + 1;
    ...
    uint32_t bonds_of_monomer = 0,
             bond_number = 0; // the current bond and the number of bonds of the
                              // current monomer
    ...
    if (start_offset_bond < 0) {
      bond_number = 0;
      bond_number_total[monomer_i - p->poly_type_offset[n_poly] - 1] =
          bond_number;
    ...
    for (unsigned int bond_i = 0; bond_i < bond_number; bond_i++) {
      ...
      bonds_total[monomer_i - p->poly_type_offset[n_poly] - 1][bond_i] =
          get_offset(bonds_of_monomer) + monomer_i -
          p->poly_type_offset[n_poly] - 1;
      start_offset_bond++;
    }
  ...
  independent_sets =
      (unsigned int **)malloc((max_bond_number + 1) * sizeof(unsigned int *));
  if (independent_sets == NULL) {
    fprintf(stderr, "Malloc problem %s:%d\n", __FILE__, __LINE__);
    return -4;
  }
  // the monomers stored between offset_set and end_set are the ones whose
  // heighbour are not stored in the sets yet
  unsigned int *end_set =
      (unsigned int *)malloc((max_bond_number + 1) * sizeof(unsigned int));
  if (end_set == NULL) {
    fprintf(stderr, "Malloc problem %s:%d\n", __FILE__, __LINE__);
    return -4;
  ...
  // loop over all monomer of a poly_type (if the chain consists several
  // molecules)
  for (unsigned int mono_i = 0; mono_i < sequence; mono_i++) {
    ...
    // find the set with least member //only help for if the chain is not
    // separated into single chains
    int start_set = 0;
    for (unsigned int set_i = 0; set_i < max_bond_number + 1; set_i++) {
    ...
    independent_sets[start_set][end_set[start_set]] =
        current_monomer; // write the central monomer into start_set
    end_set[start_set]++;
    ...
    for (int set_i = 1; set_i <= bond_number_total[current_monomer]; set_i++) {
      ...
      independent_sets[loop_set][end_set[loop_set]] =
          bonds_total[current_monomer][set_i - 1];
      total_assigned_monomer++;
    ...
    while (chain_finished == 1) {
      ...
      unsigned int writein_set =
          current_set -
          1; // which set to put the new element into (the left one)
      if (current_set == 0)
        writein_set = max_bond_number;
      // loop over all monomers in the current_set, whose neighbours are not
      // studied yet
      for (unsigned int member_set = offset_set[current_set];
           member_set < end_set[current_set]; member_set++) {
        current_monomer = independent_sets[current_set][member_set];
        for (int bond_i = 0; bond_i < bond_number_total[current_monomer];
             bond_i++) {
          if (monomer_checked[bonds_total[current_monomer][bond_i]] == -1)
            continue;
          writein_set = check_bond_members_of_set(
              bonds_total, bond_number_total, max_bond_number, writein_set,
              current_set, offset_set, end_set, independent_sets, bond_i,
              current_monomer);
  ...
  unsigned int *inde_set_tmp =
      (unsigned int *)malloc((*set_tmp_pointer)[n_poly].max_member *
                             (max_bond_number + 1) * sizeof(unsigned int));
  ...
  (*set_tmp_pointer)[n_poly].set_length =
      (unsigned int *)malloc((max_bond_number + 1) * sizeof(unsigned int));
...
unsigned int check_bond_members_of_set(
  ...
  while (found == 0) {
    ...
    for (int neighbour_index = 0;
         neighbour_index <
         bond_number_total[bonds_total[current_monomer][bond_i]];
         neighbour_index++) { // loop over all bonds
      unsigned int neighbour =
          bonds_total[bonds_total[current_monomer][bond_i]][neighbour_index];
      if (neighbour == current_monomer)
        continue;
      for (unsigned int member_i = offset_set[writein_set];
           member_i < end_set[writein_set]; member_i++) {
