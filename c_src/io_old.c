...
int read_old_config(struct Phase *p, char *const filename) {
  ...
  if (p->info_MPI.world_size !=
      1) { // Make sure this is only called for single core processes.
    fprintf(stderr,
            "ERROR: convert can only be called with a single MPI-core.\n");
    return -11;
  }
  ...
  soma_scalar_t chiN, deltaN, kappaN, stiff, hN1, gN1, fN1, phia, phib, phic,
      D0, LY, LZ, time, F, f, xiN, gdot, forceN, kNcphi, dtcphi, Neq, Nav,
      Ntdgl, kT, dummy, dt;
  char *Cdummy;
  char cdummy[1024];
  unsigned int i, j;
  ...
  iread = fscanf(inputfile,
                 ...
                 "chiN = %g; deltaN = %g; kappaN = %g; stiff = %g; hN1 = %g; "
                 "gN1 = %g; fN1 = %g; phia = %g; phib = %g; phic = %g; D0 = "
                 "%g; LY = %g; LZ = %g; N = %d %d %d; S = %d;\n",
#else  // SINGLE_PRECISION
                 "chiN = %lg; deltaN = %lg; kappaN = %lg; stiff = %lg; hN1 = "
                 "%lg; gN1 = %lg; fN1 = %lg; phia = %lg; phib = %lg; phic = "
                 "%lg; D0 = %lg; LY = %lg; LZ = %lg; N = %d %d %d; S = %d;\n",
  ...
  iread =
      fscanf(inputfile,
#if (SINGLE_PRECISION == 1)
             "time =  %g; F = %g ( %g ); f = %g; xiN = %g; dt*N/xi = %g; gdot "
             "= %g; forceN = %g; kNcphi = %g; dtcphi = %g; Neq = %g; Nav = %g; "
             "Ntdgl = %g; kT = %g; HYDRO = %d; LEBD = %d; TCHECK = %d; TWRITE "
             "= %d; VCHECK = %d;\n",
#else  // SINGLE_PRECISION
             "time =  %lg; F = %lg ( %lg ); f = %lg; xiN = %lg; dt*N/xi = %lg; "
             "gdot = %lg; forceN = %lg; kNcphi = %lg; dtcphi = %lg; Neq = %lg; "
             "Nav = %lg; Ntdgl = %lg; kT = %lg; HYDRO = %d; LEBD = %d; TCHECK "
             "= %d; TWRITE = %d; VCHECK = %d;\n",
#endif // SINGLE_PRECISION
             &time, &F, &dummy, &f, &xiN, &dt, &gdot, &forceN, &kNcphi, &dtcphi,
             &Neq, &Nav, &Ntdgl, &kT, &HYDRO, &LEBD, &TCHECK, &TWRITE, &VCHECK);
  if (iread != 19) {
    fprintf(stderr, "Old io ERROR: %s:%d. (%d)\n", __FILE__, __LINE__, iread);
    return -2;
  }
  ...
  iread = fscanf(inputfile, "%u %u %d  %u %u %d  %u %u %d\n", &NpolyA, &NmonoA,
                 &ZA, &NpolyB, &NmonoB, &ZB, &NpolyC, &NmonoC, &ZC);
  if (iread != 9) {
    fprintf(stderr, "Old io ERROR: %s:%d. (%d)\n", __FILE__, __LINE__, iread);
    return -3;
  }
  ...
  assert(p->info_MPI.world_size ==
         1); // Make sure this is only called for single core processes.
  p->n_polymers_global = p->n_polymers;
  p->n_types = 2;

  /* Assuming same diffusion constant for all monomers for this input file, but
   * value can vary! */
  ...
  p->xn =
      (soma_scalar_t *)malloc(p->n_types * p->n_types * sizeof(soma_scalar_t));
  if (p->xn == NULL) {
    fprintf(stderr, "Malloc problem %s:%d\n", __FILE__, __LINE__);
    return -4;
  }
  ...
  for (i = 0; i < p->n_polymers; i++) {
    ...
    for (j = 0; j < N; j++) {
      ...
      uint32_t *const type_info =
          &(p->poly_arch[p->poly_type_offset[p->polymers[i].type] + j + 1]);
...
int read_old_geometry(struct Phase *p, const char *filename) {
  if (p->args.N_domains_arg != 1) {
    fprintf(stderr,
            "ERROR: read_old_geometry works only with a single domain.\n");
    return -1;
  }
  ...
  if (geofile != NULL) {
    p->external_field_unified = (soma_scalar_t *)malloc(
        p->n_types * p->n_cells * sizeof(soma_scalar_t *));
    ...
    // in the original definition external fields are symmetric to the two
    // species
    for (unsigned int k = 0; k < p->n_cells; k++) {
#if (SINGLE_PRECISION == 1)
      const int iread =
          fscanf(geofile, "%d %d %d %g %g %g %g %g", &x, &y, &z, &wall, &field,
                 &dielectric, &compute_efield, &fixed_potential);
#else  // SINGLE_PRECISION
      const int iread =
          fscanf(geofile, "%d %d %d %lg %lg %lg %lg %lg", &x, &y, &z, &wall,
                 &field, &dielectric, &compute_efield, &fixed_potential);
#endif // SINGLE_PRECISION
      if (iread != 8) {
    ...
    fprintf(stderr,
            "WARNING: Passed geometry file %s unreadable. Geometry will be "
            "ignored.\n",
            filename);
    p->area51 = NULL;
    p->external_field_unified = NULL;
...
int read_beads0(struct Phase *const p, const hid_t file_id,
  ...
  if ((status = H5Dread(poly_type_dataset, H5T_NATIVE_UINT, poly_type_memspace,
  ...
  for (uint64_t i = 0; i < p->n_polymers; i++) {
    ...
    p->polymers[i].tag =
        n_polymer_offset + i; // The old file format doesn't contain tags, so we
  ...
  if (H5Lexists(file_id, "/beads", H5P_DEFAULT) > 0) {
    ...
    Monomer *const monomer_data =
        (Monomer *const)malloc(p->n_polymers * max_n_beads * sizeof(Monomer));
    ...
    if ((status = H5Dread(beads_dataset, monomer_memtype, beads_memspace,
    ...
    for (uint64_t i = 0; i < p->n_polymers; i++) {
      ...
      for (unsigned int j = 0; j < N; j++) {
        ...
        *(((Monomer *)p->ph.msd_beads.ptr) + p->polymers[i].msd_bead_offset +
          j) = data;
      }
    }
    free(monomer_data);
