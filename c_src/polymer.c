...
int reallocate_polymer_mem(struct Phase *const p, uint64_t new_storage) {
  if (p->present_on_device) {
    fprintf(stderr,
            "ERROR: %s:%d %d reallocate of poly mem, but system is present on "
            "device. Call copyout_phase first.\n",
  ...
  struct Polymer *const tmp_poly =
      (struct Polymer *const)malloc(new_storage * sizeof(struct Polymer));
  if (tmp_poly == NULL) {
...
int push_polymer(struct Phase *const p, const Polymer *const poly) {
  if (p->present_on_device) {
    fprintf(stderr,
            "ERROR: %s:%d %d PUSH, but system is present on device. Call "
            "copyout_phase first.\n",
            __FILE__, __LINE__, p->info_MPI.world_rank);
    return -1;
  }
...
int pop_polymer(struct Phase *const p, const uint64_t poly_id,
  ...
  if (p->present_on_device) {
    fprintf(stderr,
            "ERROR: %s:%d POP %d, but system is present on device. Call "
            "copyout_phase first.\n",
            __FILE__, __LINE__, p->info_MPI.world_rank);
    return -1;
  }
  ...
  if (poly_id >= p->n_polymers) {
    fprintf(stderr,
            "WARNING: Invalid pop attempt of polymer. rank: %d poly_id %ld "
            "n_polymers %ld.\n",
...
int exchange_polymer(struct Phase *const p, const uint64_t poly_i,
  ...
  if (p->present_on_device) {
    fprintf(stderr,
            "ERROR: %s:%d Exchange %d, but system is present on device. Call "
            "copyout_phase first.\n",
            __FILE__, __LINE__, p->info_MPI.world_rank);
    return -1;
  }
...
int serialize_polymer(struct Phase *const p, const Polymer *const poly,
  ...
  // Beads data
  memcpy(buffer + position, ((Monomer *)p->ph.beads.ptr) + poly->bead_offset,
         N * sizeof(Monomer));
  position += N * sizeof(Monomer);

  // MSD data
  memcpy(buffer + position,
         ((Monomer *)p->ph.msd_beads.ptr) + poly->msd_bead_offset,
         N * sizeof(Monomer));
  position += N * sizeof(Monomer);
  ...
  if (poly->set_permutation_offset != UINT64_MAX) {
    memcpy(buffer + position,
           ((unsigned int *)p->ph.set_permutation.ptr) +
               poly->set_permutation_offset,
  ...
  if (poly->set_states_offset != UINT64_MAX)
    for (unsigned int i = 0; i < p->max_set_members; i++)
      position += serialize_rng_state(
          p, ((RNG_STATE *)p->ph.set_states.ptr) + poly->set_states_offset + i,
          buffer + position);
...
int deserialize_polymer(struct Phase *const p, Polymer *const poly,
  ...
  if (length < sizeof(unsigned int)) {
    fprintf(stderr, "ERROR: %s:%d:%d invalid buffer received %d \n", __FILE__,
            __LINE__, p->info_MPI.world_rank, length);
    return length;
  }
  ...
  memcpy(((Monomer *)p->ph.beads.ptr) + poly->bead_offset, buffer + position,
         N * sizeof(Monomer));
  position += N * sizeof(Monomer);
  ...
  memcpy(((Monomer *)p->ph.msd_beads.ptr) + poly->msd_bead_offset,
         buffer + position, N * sizeof(Monomer));
  position += N * sizeof(Monomer);
  ...
  if (length > position) {
    ...
    memcpy(((unsigned int *)p->ph.set_permutation.ptr) +
               poly->set_permutation_offset,
           buffer + position, p->max_n_sets * sizeof(unsigned int));
    position += p->max_n_sets * sizeof(unsigned int);
  }

  if (length > position) {
    poly->set_states_offset =
    ...
    for (unsigned int i = 0; i < p->max_set_members; i++)
      position += deserialize_rng_state(
          p, ((RNG_STATE *)p->ph.set_states.ptr) + poly->set_states_offset + i,
          buffer + position);
  } else {
  ...
  if (position != length) {
    fprintf(stderr,
            "ERROR: %s:%d rank %d Deserialization of polymer. "
