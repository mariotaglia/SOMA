...
//!
//! Option to select the pseudo random number  enerator  (possible
//! values="PCG32", "MT" default=`PCG32')
//! Defualt is PCG32. For every other *alternative* RNG the state only stores an
//! offset to the global array of RNGs. See  rng_alternative.h for details.
typedef struct RNG_STATE {
  PCG_STATE default_state; //!<\brief PCG32
  uint64_t
      alternative_rng_offset; //!< offset to read in the global memory
                              //!< (Depending on the switch in phase indicating
                              //!< the RNG to be used, this offsets to
                              //!< different. UINT64_MAX if invalid
} RNG_STATE;
...
//! \param rng struct which contains all information about the internal states
//! of the rngs \param p Phase struct of the simulated system \pre rng has been
//! seeded. \return prng in range [0,1)
...
//! \param rng struct which contains all information about the internal states
//! of the rngs \param p Phase struct of the simulated system \param x
//! coordinate of the vector \param y coordinate of the vector \param z
//! coordinate of the vector \pre rng has been seeded
#pragma acc routine(soma_normal_vector) seq
void soma_normal_vector(RNG_STATE *rng, const struct Phase *const p,
                        soma_scalar_t *x, soma_scalar_t *y, soma_scalar_t *z);
...
//! \pre Allocation of buffer with return value of rng_state_serial_length()
//! minimum. \note Ownership and allocation status is unchanged. \return Number
//! of written bytes. If < 0 Errorcode.
int serialize_rng_state(struct Phase *const p, const RNG_STATE *const state,
...
//! \post You are owner of the state including deep copy data, because deep copy
//! data is allocated. \return Number of written bytes. If < 0 Errorcode.
int deserialize_rng_state(struct Phase *const p, RNG_STATE *const state,
...
//! \return Errorcode
int seed_rng_state(struct RNG_STATE *const state, const unsigned int seed,
                   const unsigned int stream, const struct Phase *const p);
