...
int consider_compact_polymer_heavy(struct Phase *p, const bool collective) {
  ...
  if (collective)
    MPI_Allreduce(MPI_IN_PLACE, &compact, 1, MPI_INT, MPI_LOR,
  ...
  if (permutation_device &&
      (p->args.iteration_alg_arg == iteration_alg_arg_SET))
    copyout_soma_memory(&(p->ph.set_permutation));

  PolymerHeavy new_ph;
  if (init_soma_memory(&(new_ph.beads), p->num_all_beads_local,
                       sizeof(Monomer)) != 0) {
    fprintf(stderr, "ERROR compacting beads\n");
    return -1;
  }
  if (init_soma_memory(&(new_ph.msd_beads), p->num_all_beads_local,
                       sizeof(Monomer)) != 0) {
    fprintf(stderr, "ERROR compacting msd_beads\n");
    return -2;
  }
  if (p->args.iteration_alg_arg == iteration_alg_arg_SET) {
    if (init_soma_memory(&(new_ph.set_states),
                         p->n_polymers * p->max_set_members,
                         sizeof(RNG_STATE)) != 0) {
      fprintf(stderr, "ERROR compacting set_states\n");
      return -3;
    }
    if (init_soma_memory(&(new_ph.set_permutation),
                         p->n_polymers * p->max_n_sets,
  ...
  for (uint64_t i = 0; i < p->n_polymers; i++) {
    ...
    uint64_t new_msd_bead_offset =
        get_new_soma_memory_offset(&(new_ph.msd_beads), N);
    ...
    if (p->args.iteration_alg_arg == iteration_alg_arg_SET) {
      uint64_t new_state_offset =
          get_new_soma_memory_offset(&(new_ph.set_states), p->max_set_members);
  ...
  fprintf(stdout,
          "World rank %d compacts memory at time step %d. Balance %lu old %lu "
          "new %lu\n",
          p->info_MPI.world_rank, p->time, p->num_all_beads_local,
          p->ph.beads.length, new_ph.beads.length);
