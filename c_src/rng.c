...
unsigned int soma_rng_uint_max() { return 4294967295U; }
...
soma_scalar_t soma_rng_soma_scalar(RNG_STATE *rng,
...
unsigned int rng_state_serial_length(const struct Phase *const p) {
  ...
  if (p->args.pseudo_random_number_generator_arg ==
      pseudo_random_number_generator_arg_MT)
...
int serialize_rng_state(struct Phase *const p, const RNG_STATE *const state,
  ...
  if (p->args.pseudo_random_number_generator_arg ==
    ...
    memcpy(buffer + position,
           ((MERSENNE_TWISTER_STATE *)p->rh.mt.ptr) +
  ...
  if (p->args.pseudo_random_number_generator_arg ==
      pseudo_random_number_generator_arg_TT800) {
    memcpy(buffer + position,
           ((TT800STATE *)p->rh.tt800.ptr) + state->alternative_rng_offset,
           sizeof(TT800STATE));
...
int deserialize_rng_state(struct Phase *const p, RNG_STATE *const state,
  ...
  if (p->args.pseudo_random_number_generator_arg ==
    ...
    memcpy(((MERSENNE_TWISTER_STATE *)p->rh.mt.ptr) +
               state->alternative_rng_offset,
           buffer + position, sizeof(MERSENNE_TWISTER_STATE));
    position += sizeof(MERSENNE_TWISTER_STATE);
  }
  ...
  if (p->args.pseudo_random_number_generator_arg ==
    ...
    memcpy(((TT800STATE *)p->rh.tt800.ptr) + state->alternative_rng_offset,
           buffer + position, sizeof(TT800STATE));
    position += sizeof(TT800STATE);
  }
...
int seed_rng_state(struct RNG_STATE *const state, const unsigned int seed,
  ...
  case pseudo_random_number_generator_arg_MT:;
    MERSENNE_TWISTER_STATE *mt_state = p->rh.mt.ptr;
    soma_seed_rng_mt(&(state->default_state),
                     mt_state + state->alternative_rng_offset);
    break;
  case pseudo_random_number_generator_arg_TT800:;
    TT800STATE *tt800_state = p->rh.tt800.ptr;
    soma_seed_rng_tt800(&(state->default_state),
                        tt800_state + state->alternative_rng_offset);
    break;
  } // end switch
  return 0;
