...
enum Hamiltonian {
  SCMF0 = 0, //!< Original SCMF hamiltonian. For details refer doc of
             //!< update_omega_fields_scmf0().
  SCMF1 = 1, //!< Alternative SCMF hamiltonian, especially for more than 2
             //!< types. For details refer doc of update_omega_fields_scmf1().
};
...
enum MobilityEnum {
  DEFAULT_MOBILITY =
      0, //!< Default unchanged mobility. pacc multiplied with 1. Always.
  MULLER_SMITH_MOBILITY =
      1, //!< Mobility reduction factor of the shape \f$ m(r) =
         //!< \min(1,\sum_\alpha a_\alpha \phi_\alpha(r) + \sum_alpha b_\alpha
         //!< \phi_alpha^2(r)) \f$. With parameters a and b to be set via
         //!< xml/hdf5.
  //! Mobility reduction with the shape \f$ M_i(\{\phi_j\}) = 1/2 \left( 1 +
  //! \tanh\left(\frac{\phi_{0,i} - \sum_j a_{ij} \phi_j}{\Delta
  //! \phi_{i}}\right)\right) \f$
  TANH_MOBILITY = 2,
};
...
//!  Function to extract the offset you apply to your Monomer to get the bonded
//!  neigbour.
//!
//! \param info poly_arch element.
...
//!
//! \param info poly_arch element.
//! \returns End flag of the bond list. Stop iteration over the bond list, if it
//! is non zero. \warning Call only for bond list part of poly_arch.
#pragma acc routine seq
...
#pragma acc routine seq
uint32_t get_info(const int offset, const unsigned int bond_type,
...
//! \brief after argument parsing of SOMA, this function interpret
//! contradictions and warnings for the user.
//!
...
//! \brief Returns the number of bonds of specific type in the poly_arch
//! structure. \param p Phase of the system. \param btype Bondtype to count
//! \warning This does not count the number of bonds in the system, only which
//! bond types are used in the polyarch structure. \return Number of bonds in
//! the poly_arch structure. You may you that function to check if a specific
//! bond type is present in the system.
unsigned int get_number_bond_type(const struct Phase *const p,
                                  const enum Bondtype btype);
...
#define HDF5_ERROR_CHECK(status)                                               \
  if (status < 0) {                                                            \
    fprintf(stderr, "ERROR: HDF5 %s:%s:%d: %d\n", __func__, __FILE__,          \
            __LINE__, (int)status);                                            \
    return status;                                                             \
  }
...
#define HDF5_ERROR_CHECK2(status, name)                                        \
  if (status < 0) {                                                            \
    fprintf(stderr, "ERROR: HDF5 Name:%s %s:%d: %d\n", name, __FILE__,         \
            __LINE__, (int)status);                                            \
    return status;                                                             \
  }

//! Macro to abort MPI and print a message with you
//! specified if status != 0. \warning This macro includes finalize_MPI() and
//! exit();
#define MPI_ERROR_CHECK(status, msg)                                           \
  if (status != 0) {                                                           \
    fprintf(stderr, "ERROR: MPI abort Name: %s %s:%d: %d\n", msg, __FILE__,    \
            __LINE__, (int)status);                                            \
    ;                                                                          \
    exit(status);                                                              \
  }
...
#define MALLOC_ERROR_CHECK(ptr, size)                                          \
  if ((ptr) == NULL) {                                                         \
    fprintf(stderr, "MALLOC-ERROR: %s:%d size = %lu\n", __FILE__, __LINE__,    \
            (uint64_t)(size));                                                 \
    return -1;                                                                 \
  }
#pragma acc routine(calc_bond_length) seq
static inline soma_scalar_t calc_bond_length(const soma_scalar_t x_i,
                                             const soma_scalar_t x_j,
                                             const soma_scalar_t box,
                                             const int mic);
inline soma_scalar_t calc_bond_length(const soma_scalar_t x_i,
                                      const soma_scalar_t x_j,
                                      const soma_scalar_t box, const int mic) {
  soma_scalar_t r =
      x_i - x_j + 0 * mic * box; // after "+" to shut up compiler warnings
#if (ENABLE_MIC == 1)
  if (mic) {
