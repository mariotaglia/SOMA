...
void calc_dielectric_field(struct Phase *const p)
{
    ...
    soma_scalar_t tmp_phi_n;
    soma_scalar_t tmp_eps_phi_n;
...
#pragma omp parallel
    for (i = 0; i < p-> n_cells; i++)
    {
        ...
        else
        {
            ...
            // Calculation according to welling2014 eq. 82 eq. 83, density independent expression
            tmp_phi_n = 0.0;
            tmp_eps_phi_n = 0.0;
            ...
            for (uint8_t n = 0; n < p->n_types; n++)
            {
                ...
                tmp_eps_phi_n += tmp_phi_n * p->ef.eps[n];
            }

            p->ef.eps_arr[i] = eps_0 * tmp_eps_phi_n / tmp_phi_n;
        }
    }
}
...
int calc_electric_field_contr(struct Phase *const p)
{
    ...
    soma_scalar_t dEpot_sq, eps_res, phi_other, phi_self, eps_diff;
    uint64_t k = 0;
        ...
        for (y=0; y < p->ny; y++)
            for (z=0; z < p->nz; z++)
            {
                ...
                else
                {
                    ...
                    for (m = 0; m < p->n_types; m++)
                    {   
                        eps_res = 0.0;
                        phi_other = 0.0;
                        for (n = 0; n < p->n_types; n++)
                        {
                            if (n != m)
                            {
                                eps_res += p->ef.eps[n];
