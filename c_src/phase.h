...
typedef struct Phase {
  bool present_on_device; //!< Indicator if the whole system is present of the
                          //!< GPU.
  unsigned int reference_Nbeads; /*!< \brief number of reference beads for the
                                    model polymer */
  soma_scalar_t harmonic_normb;  //!< Harmonic energy scale (function of spring
                                 //!< constant) const. at runtime.
  soma_scalar_t harmonic_normb_variable_scale; //!< different harmonic energy
                                               //!< scale (function of spring
                                               //!< constant) const. at runtime.
  ...
  soma_scalar_t *xn;
  uint64_t n_polymers; /*!< \brief \#polymers in the configuration. (Local on
                          the MPI node.) */
  uint64_t n_polymers_storage; /*!< \brief Storage space for polymers. */
  uint64_t
      n_polymers_global; /*!< \brief \#polymers in the global configuration. */
  Polymer *polymers;     /*!< \brief pointer to array of polymers */

  // uint16_t **fields; /*!< \brief n_types fields in 3D, mimics the DENSITY NOT
  // normalized to 1, this has to be done in the omega_field calculation*/
  uint16_t *fields_unified; /*!< \brief one pointer that points to the construct
                               of p->n_types * p->n_cells_local of fields */
  uint16_t
      *old_fields_unified; /*!< \brief one pointer that points to the construct
                              of p->n_types * p->n_cells_local of old fields for
                              density variance calculations */
  uint32_t *fields_32; //!< \brief linear 32 bit version of the fields. This is
                       //!< required for GPU-simulation, because no 16-bit
                       //!< atomic operations are available.

  /*! \brief array of shape of field, containing the information if
   * this cell is free space or not. == 0  is a free cell, !=0 is a forbidden
   * cell. If the pointer is set to NULL, the entire simulation box, contains no
   * forbidden cells.
   *
  ...
  // soma_scalar_t ** omega_field; /*!< \brief calculates the omega fields
  // according to the Hamiltonian*/
  soma_scalar_t
      *omega_field_unified; /*!< \brief calculates the omega fields according to
                               the Hamiltonian, unified access */
  // soma_scalar_t ** external_field; /*!< \brief external fields that act on
  // the polymers, one field per type */
  soma_scalar_t
      *external_field_unified; /*!< \brief one pointer that points to the
                                  construct of p->n_types * p->n_cells_local of
                                  external_fields */
  soma_scalar_t
      *umbrella_field; /*!< \brief one pointer that points to the construct of
                          p->n_types * p->n_cells_local of umbrella_field */
  soma_scalar_t
      *tempfield; /*!< \brief a temporal storage for intermediate field
                     calculations, used to save the complete density */

  soma_scalar_t *A; /*!< \brief stores the diffusion constants for each type */
  soma_scalar_t
      *R; /*!< \brief stores the derived dR for the diffusion constant */
  ...
  soma_scalar_t *field_scaling_type; /*!< \brief stores the scaling factor
                                        according to the density */

  soma_scalar_t
      *k_umbrella; //!< Strength prefactor for the umbrella Hamiltonians for
                   //!< each type, if needed. Default 0
  ...
  uint64_t num_all_beads_local; //!< Number of all monomer/beads on the local
                                //!< MPI core
  uint64_t n_accessible_cells;  //!< Global number of cells of the grid that are
                                //!< accessible by particles (not-area51)
  ...
  soma_scalar_t
      iLx; /*!< \brief inverse x-spatial dimensions in units of \f$ Re_0 \f$ */
  soma_scalar_t
      iLy; /*!< \brief inverse y-spatial dimensions in units of \f$ Re_0 \f$ */
  soma_scalar_t
      iLz; /*!< \brief inverse z-spatial dimensions in units of \f$ Re_0 \f$ */
  ...
  uint16_t *left_tmp_buffer;  //!< If not NULL: Temporary memory of size
                              //!< domain_buffer*ny*nz for MPI communication
  uint16_t *right_tmp_buffer; //!< If not NULL: Temporary memory of size
                              //!< domain_buffer*ny*nz for MPI communication
  ...
  //!
  //! \warning The useage of this array is \a not straight forward. Please read
  //! the doc carefully.
  //!
  //! The array contains three different information about the polymer
  //! architecture.
  //!  - The number of monomers per polymer. This information is always located
  //!  at poly_arch[poly_type_offset[poly_type]].
  //!    You can read this information directly:
  //!    \code const unsigned int N = poly_arch[poly_type_offset[poly_type]];
  //!    \endcode Writing is not recomeneded.
  //!  - The information about the bonded neighbours is stored in a so called
  //!  bond_offset_lists.<br>
  ...
  //!                                  p->poly_arch[poly_type_offset[poly_type]
  //!                                  + mono_index + 1] );
  //!    \endcode
  ...
  //!    is last element in the bond list. You can access the ith element of
  //!    your bondlist via: \code const unsigned int end = get_end(
  //!    p->poly_arch[ start + i] );\endcode
  ...
  //! Array of independet sets. If NULL no sets are stored, the move type is not
  //! available.
  //!
  //! Length of the array is the number of polymer types.
  struct IndependetSets *sets;
  //! Number of max members in an independet set. Used for the length of polymer
  //! states.
  ...
  soma_scalar_t *cos_serie;  //!< the prefactor of the cosine elements to define
                             //!< time-dependent external field
  soma_scalar_t *sin_serie;  //!< the prefactor of the sine elements to define
                             //!< time-dependent external field
  ...
  struct PolyConversion
      pc; //!< struct containing the information for the poly type convsersion
  struct Mobility mobility;    //!< struct containing information for density
                               //!< related mobility modifications
  struct SelfDocumentation sd; //!< struct that contains all elements for the
                               //!< self documenation functionality
  struct PolymerHeavy ph; //!< struct containing the pointer to the heavy memory
                          //!< of the polymers.
  struct RNG_HEAVY rh; //!< struct containing the pointer to the heavy memory of
                       //!< alternative PRNGs, if needed.
} Phase;

/*! \brief Initializes the values additional after the input init by the read*()
  functions.

  \note This function should be called before any type of simulaton,
  because several parameters from the input are used to fully initialize the
  Phase. \param p Phase to initialize. \return error code.
*/
...
/*! \brief Deallocate the memory used by p.
  ...
  \return Errorcode.
  \post p points to an invalid configuration. Do not use it for any other call
  afterwards.
*/
...
/*! \brief Order the polymers in p according to their length, private function.
  \param p Initialized configuration.
  \return the number of chains that have more than 1/50 of the monomers in the
  total system.
*/
