...
int read_mobility_hdf5(struct Phase *const p, const hid_t file_id,
  ...
  p->mobility.poly_type_mc_freq =
      (unsigned int *)malloc(p->n_poly_type * sizeof(unsigned int));
  MALLOC_ERROR_CHECK(p->mobility.poly_type_mc_freq,
                     p->n_poly_type * sizeof(unsigned int));
  ...
  status =
      read_hdf5(file_id, "mobility/type", H5T_STD_U32LE, plist_id, &tmp_type);
  HDF5_ERROR_CHECK(status);

  enum MobilityEnum tmp_enum_type = tmp_type;

  // Read the poly_type_mc_freq in any case
  status = read_hdf5(file_id, "mobility/poly_type_mc_freq", H5T_STD_U32LE,
                     plist_id, p->mobility.poly_type_mc_freq);
  HDF5_ERROR_CHECK(status);
  for (unsigned int i = 0; i < p->n_poly_type; i++)
    if (p->mobility.poly_type_mc_freq[i] == 0) {
      fprintf(stderr,
              "ERROR: %s:%d poly_type_mc_freq contains a 0. This is "
              "illdefined. min value is 1.\n",
              __FILE__, __LINE__);
      return -2;
    }
  ...
  if (ndims_param != 1) {
    fprintf(stderr,
            "ERROR: %s:%d not the correct number of dimensions for %s.\n",
  ...
  case MULLER_SMITH_MOBILITY:
    if (dim_param != 2 * p->n_types) {
      fprintf(stderr,
              "ERROR: %s:%d invalid number of mobility parameter for "
              "MULLER_SMITH\n",
              __FILE__, __LINE__);
      fprintf(stderr,
              "\tExpected number of parameters %d but got %d parameters.\n",
              2 * p->n_types, (int)dim_param);
      return -2;
    }
    break;
  case TANH_MOBILITY:
    if (dim_param != 2 * p->n_types + p->n_types * p->n_types) {
      fprintf(stderr,
              "ERROR: %s:%d invalid number of mobility parameter for TANH\n",
              __FILE__, __LINE__);
      fprintf(stderr,
              "\tExpected number of parameters %d but got %d parameters.\n",
              2 * p->n_types, (int)dim_param);
  ...
  default:
    fprintf(stderr,
            "ERROR: reading invalid mobility type. Old SOMA version? %s:%d\n",
            __FILE__, __LINE__);
    return -1;
  }

  p->mobility.param =
      (soma_scalar_t *)malloc(dim_param * sizeof(soma_scalar_t));
  ...
  status = H5Dread(dset_param, H5T_SOMA_FILE_SCALAR, H5S_ALL, H5S_ALL, plist_id,
                   p->mobility.param);
  HDF5_ERROR_CHECK(status);
...
int write_mobility_hdf5(const struct Phase *const p, const hid_t file_id,
  ...
  hid_t group =
      H5Gcreate2(file_id, "/mobility", H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
  herr_t status;
  const hsize_t one = 1;
  unsigned int tmp_type = p->mobility.type;
  status = write_hdf5(1, &one, file_id, "/mobility/type", H5T_STD_U32LE,
                      H5T_NATIVE_UINT, plist_id, &(tmp_type));
  HDF5_ERROR_CHECK(status);

  const hsize_t n_poly_type = p->n_poly_type;
  status = write_hdf5(1, &n_poly_type, file_id, "mobility/poly_type_mc_freq",
                      H5T_STD_U32LE, H5T_NATIVE_UINT, plist_id,
                      p->mobility.poly_type_mc_freq);

  const hsize_t list_len = p->mobility.param_len;
  status =
      write_hdf5(1, &list_len, file_id, "/mobility/param", H5T_SOMA_FILE_SCALAR,
                 H5T_SOMA_NATIVE_SCALAR, plist_id, p->mobility.param);
  HDF5_ERROR_CHECK(status);
...
soma_scalar_t get_mobility_modifier(const struct Phase *const p,
  ...
  case MULLER_SMITH_MOBILITY:;
    ...
    for (unsigned int type = 0; type < p->n_types; type++) {
      ...
      const soma_scalar_t phi =
          p->field_scaling_type[type] * p->fields_unified[cellindex];
  ...
  case TANH_MOBILITY:;
    ...
    for (unsigned int j = 0; j < p->n_types; j++) {
      ...
      a_sum += a[particle_type * p->n_types + j] * p->field_scaling_type[j] *
               p->fields_unified[cellindex];
    }

    return 0.5 * (1 + tanh((phi_0[particle_type] - a_sum) /
                           delta_phi[particle_type]));
    break;
  }
