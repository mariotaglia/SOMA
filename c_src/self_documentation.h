...
//! \brief Define structs and function to enable user friendly documentation of
//! the history and self of simulations.
...
//!
//! This info is read of the old configuration file and updated with the next
//! simulation info. Every configuration file written during the simulation will
//! contain this info.
typedef struct SelfDocumentation {
  char **simdoc; //!< Array of string to self of the simulation. The first
                 //!< dimension counts every simulation start, the second is the
                 //!< string
  unsigned int Ndoc;     //!< length of the self_doc. How many simulation
                         //!< self-documenting strings are stored.
  char *simdoc_internal; //!< internal memory to hold all the string in a single
                         //!< array
} SelfDocumentation;
...
//! \param filename Filename of HDF5, if NULL is passed a fresh
//! SelfDocumentation is initialized \param sd SelfDocumentation Pointer to
//! struct to init \return Errorcode
int init_self_documentation(struct Phase *p, char *filename,
                            struct SelfDocumentation *sd);

//! Write the self documentation (including the history) to an already opened
//! HDF5 file.
//!
//! If the documentation dataset is present in the file, unlink the old and
//! generate a new one \param sd Initialized SelfDocumentation to write to file
