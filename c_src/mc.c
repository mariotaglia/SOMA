...
void trial_move_cm(const Phase *p, const uint64_t poly_type,
...
int som_accept(RNG_STATE *const rng, const Phase *const p,
...
soma_scalar_t
calc_delta_nonbonded_energy(const Phase *p, const Monomer *monomer,
  ...
  const uint64_t cellindex_old =
      coord_to_index_unified(p, xold, yold, zold, iwtype);
  const uint64_t cellindex_new =
      coord_to_index_unified(p, xold + dx, yold + dy, zold + dz, iwtype);
  if (cellindex_old > p->n_cells_local * p->n_types ||
      cellindex_new > p->n_cells_local * p->n_types) {
#ifdef NAN
#ifdef __PGI
#if __PGIC__ < 20
    return 0 / 0;
...
soma_scalar_t calc_delta_energy(const Phase *p, const uint64_t ipoly,
                                const Monomer *const monomer,
                                const unsigned int ibead,
                                const soma_scalar_t dx, const soma_scalar_t dy,
                                const soma_scalar_t dz,
                                const unsigned int iwtype) {
  const soma_scalar_t delta_bonded_energy =
      calc_delta_bonded_energy(p, monomer, ipoly, ibead, dx, dy, dz);
  const soma_scalar_t delta_nonbonded_energy =
      calc_delta_nonbonded_energy(p, monomer, dx, dy, dz, iwtype);
...
soma_scalar_t calc_delta_bonded_energy(const Phase *p, const Monomer *monomer,
                                       const uint64_t ipoly,
                                       const unsigned int ibead,
                                       const soma_scalar_t dx,
                                       const soma_scalar_t dy,
                                       const soma_scalar_t dz) {
...
int monte_carlo_propagation(Phase *const p, unsigned int nsteps) {
  ...
  if (p->args.autotuner_restart_period_arg > 0 &&
      p->time % p->args.autotuner_restart_period_arg == 0) {
...
int mc_center_mass(Phase *const p, const unsigned int nsteps,
...
#pragma omp parallel for reduction(+ : n_accepts)
    for (uint64_t npoly = 0; npoly < n_polymers; npoly++) {
      ...
      if (p->cm_a[poly_type] > 0) {
        ...
        //#pragma acc loop vector reduction(+:delta_energy)
        // reduction(|:move_allowed) Unfortunately this has to be a seq loop,
        // because the reduction crashes.
#pragma acc loop vector seq
        for (unsigned int ibead = 0; ibead < myN; ibead++) {
          ...
          const unsigned int iwtype = get_particle_type(
              p->poly_arch[p->poly_type_offset[poly_type] + 1 + ibead]);

          const int tmp =
              possible_move_area51(p, mybead.x, mybead.y, mybead.z, dx, dy, dz,
          ...
          if (tmp) {
            ...
            pacc_modifier *= get_mobility_modifier(
                p, iwtype, mybead.x + dx, mybead.y + dy, mybead.z + dz);
          }
        }
        if (delta_energy !=
            delta_energy) // isnan(delta_energy) ) not working with PGI OpanACC
        {
          error_flags[0] = npoly + 1;
          move_allowed = 0;
        }

        // Take the sqaure root of mobility modifier m (collectively for all
        // beads)
        pacc_modifier = sqrt(pacc_modifier);

        // Accept Monte-Carlo call
        if (move_allowed &&
            som_accept(myrngstate, p, delta_energy, pacc_modifier) == 1) {
#ifndef _OPENACC
          n_accepts += 1;
#endif //_OPENACC
...
int mc_polymer_iteration(Phase *const p, const unsigned int nsteps,
                         const unsigned int tuning_parameter) {
  // Shutup compiler warning
  unsigned int step = tuning_parameter;
  step = 0;
...
#pragma omp parallel for reduction(+ : n_accepts)
    for (uint64_t npoly = 0; npoly < n_polymers; npoly++) {
      ...
      // Rebuild bond information for this chain from bonds, or stay with linear
      // right now?
      Polymer *mypoly = &p->polymers[npoly];
      ...
      RNG_STATE *myrngstate =
          &mypoly->poly_state; // maybe local copy of rngstate
...
#pragma acc loop seq
      for (unsigned int nmc = 0; nmc < myN; nmc++) {
        ...
        const unsigned int iwtype = get_particle_type(
            p->poly_arch[mypoly_poly_type_offset + 1 + ibead]);
        ...
        case move_type_arg_TRIAL:
          trial_move(p, npoly, ibead, &dx, &dy, &dz, iwtype,
                     myrngstate); // normal MC move
          delta_E_bond =
              calc_delta_bonded_energy(p, &mybead, npoly, ibead, dx, dy, dz);
          break;
        case move_type_arg_SMART:
          trial_move_smc(p, npoly, ibead, &dx, &dy, &dz, &delta_E_bond, &mybead,
                         myrngstate, iwtype); // force biased move
          break;
        ...
        const int move_allowed =
            possible_move_area51(p, mybead.x, mybead.y, mybead.z, dx, dy, dz,
                                 p->args.nonexact_area51_flag);
        if (move_allowed) {
          delta_energy = delta_E_bond;
          delta_energy +=
              calc_delta_nonbonded_energy(p, &mybead, dx, dy, dz, iwtype);
          if (delta_energy != delta_energy) // isnan(delta_energy) ) not working
                                            // with PGI OpenACC
          {
            error_flags[0] = npoly + 1;
          }
...
int set_iteration_multi_chain(Phase *const p, const unsigned int nsteps,
                              const unsigned int tuning_parameter,
                              const int nonexact_area51,
...
#pragma acc loop seq
      for (unsigned int i = 0; i < n_sets; i++) {
        const unsigned int d =
            soma_rng_uint(&(mypoly->poly_state), p) % (i + 1);
...
#pragma acc loop vector
        for (unsigned int iP = 0; iP < len; iP++) {
          ...
          const unsigned int iwtype = get_particle_type(
              p->poly_arch[mypoly_poly_type_offset + 1 + ibead]);
          int error_0 = set_iteration_possible_move(
              p, set_states, beads, npoly, iP, nonexact_area51, ibead, iwtype,
              &accepted_moves_set);
...
int set_iteration_single_chain(Phase *const p, const unsigned int nsteps,
                               const unsigned int tuning_parameter,
...
#pragma acc enter data copyin(error_flags [0:2])
  for (unsigned int step = 0; step < nsteps; step++) {
    ...
    for (unsigned int i = 0; i < n_sets; i++) {
      const unsigned int d =
          soma_rng_uint(&((&p->polymers[chain_i])->poly_state), p) % (i + 1);
      set_permutation[i] = set_permutation[d];
      set_permutation[d] = i;
    }
...
#pragma omp parallel for reduction(+ : accepted_moves_set)
      for (unsigned int iP = 0; iP < len; iP++) {
        ...
        const unsigned int iwtype = get_particle_type(
            p->poly_arch[mypoly_poly_type_offset + 1 + ibead]);
        int error_0 = set_iteration_possible_move(p, set_states, beads, chain_i,
                                                  iP, nonexact_area51, ibead,
                                                  iwtype, &accepted_moves_set);
        error_flags[0] = error_0;
      }
...
int mc_set_iteration(Phase *const p, const unsigned int nsteps,
                     const unsigned int tuning_parameter) {
  const int nonexact_area51 = p->args.nonexact_area51_flag +
                              0 * tuning_parameter; //&Shutup compiler warning.

  // test the order of the polymers and reorder the polymers according to their
  // length if needed
  if (p->time % p->args.set_order_frequency_arg == 0) {
    int order = 0;
    uint32_t *const length_poly =
        (uint32_t *const)malloc(p->n_polymers * sizeof(uint32_t));
    if (length_poly == NULL) {
  ...
  for (unsigned int index = 0; index < num_long_chain; index++) {
    ret = set_iteration_single_chain(p, nsteps, tuning_parameter,
                                     nonexact_area51, index);
  }
  if (num_long_chain != p->n_polymers) {
    ret = set_iteration_multi_chain(p, nsteps, tuning_parameter,
                                    nonexact_area51, num_long_chain);
  }
  p->time += 1;
#pragma acc wait
  ret = 0;
...
void trial_move_smc(const Phase *p, const uint64_t ipoly, const int ibead,
                    soma_scalar_t *dx, soma_scalar_t *dy, soma_scalar_t *dz,
                    soma_scalar_t *delta_E_bond, const Monomer *mybead,
  ...
  propose_smc_move(p, ipoly, ibead, iwtype, x, y, z, rx, ry, rz, &E_bond, dx,
                   dy, dz);
  *delta_E_bond = E_bond;
}

void propose_smc_move(const Phase *p, const uint64_t ipoly,
  ...
  if (start > 0) {
    ...
    for (int i = start; end == 0; i++) {
      ...
      case HARMONIC:
        ...
        soma_scalar_t bx_tmp =
            calc_bond_length(beads[jbead].x, x, p->Lx,
                             p->args.bond_minimum_image_convention_flag);
        soma_scalar_t by_tmp =
            calc_bond_length(beads[jbead].y, y, p->Ly,
                             p->args.bond_minimum_image_convention_flag);
        soma_scalar_t bz_tmp =
            calc_bond_length(beads[jbead].z, z, p->Lz,
                             p->args.bond_minimum_image_convention_flag);
        ...
        soma_scalar_t old_r2_tmp =
            bx_tmp * bx_tmp + by_tmp * by_tmp + bz_tmp * bz_tmp;
  ...
  if (start > 0) {
    ...
    for (int i = start; end == 0; i++) {
      ...
      case HARMONIC:
        ...
        soma_scalar_t bx_tmp =
            calc_bond_length(beads[jbead].x, x + *dx, p->Lx,
                             p->args.bond_minimum_image_convention_flag);
        soma_scalar_t by_tmp =
            calc_bond_length(beads[jbead].y, y + *dy, p->Ly,
                             p->args.bond_minimum_image_convention_flag);
        soma_scalar_t bz_tmp =
            calc_bond_length(beads[jbead].z, z + *dz, p->Lz,
                             p->args.bond_minimum_image_convention_flag);
  ...
  *delta_E_bond = new_E_B - old_E_B;
  soma_scalar_t delta_E_SMC =
      0.5 * ((nfx + fx) * (*dx) + (nfy + fy) * (*dy) + (nfz + fz) * (*dz)) +
      0.25 * A *
          ((nfx * nfx) + (nfy * nfy) + (nfz * nfz) - (fx * fx) - (fy * fy) -
           (fz * fz));
  *delta_E_bond += delta_E_SMC;
}

void add_bond_forces(const Phase *p, const uint64_t ipoly,
  ...
  if (start > 0) {
    ...
    for (int i = start; end == 0; i++) {
      ...
      case HARMONIC:
          ...
          // has to come before any declaration
          ;
        soma_scalar_t v1x_tmp =
            calc_bond_length(beads[jbead].x, x, p->Lx,
                             p->args.bond_minimum_image_convention_flag);
        soma_scalar_t v1y_tmp =
            calc_bond_length(beads[jbead].y, y, p->Ly,
                             p->args.bond_minimum_image_convention_flag);
        soma_scalar_t v1z_tmp =
            calc_bond_length(beads[jbead].z, z, p->Lz,
                             p->args.bond_minimum_image_convention_flag);
...
inline int possible_move_area51(const Phase *p, const soma_scalar_t oldx,
                                const soma_scalar_t oldy,
                                const soma_scalar_t oldz, soma_scalar_t dx,
                                soma_scalar_t dy, soma_scalar_t dz,
                                const int nonexact) {
  if (p->area51 == NULL)
    return 1;
  ...
  if (idx >= p->n_cells_local)
    return 0; // Domain error. Silent, but delta_E should give the appropriate
              // signaling.
...
int set_iteration_possible_move(const Phase *p, RNG_STATE *const set_states,
  ...
  switch (p->args.move_type_arg) {
  case move_type_arg_TRIAL:
    trial_move(p, chain_index, ibead, &dx.x, &dx.y, &dx.z, iwtype,
               &my_state); // normal MC move
    delta_E_bond = calc_delta_bonded_energy(p, &mybead, chain_index, ibead,
                                            dx.x, dx.y, dx.z);
    break;
  case move_type_arg_SMART:
    trial_move_smc(p, chain_index, ibead, &dx.x, &dx.y, &dx.z, &delta_E_bond,
                   &mybead, &my_state, iwtype); // force biased move
    break;
  case move_type__NULL:
  default:
    delta_E_bond = 0.0;
  ...
  if (move_allowed) {
    ...
    delta_energy +=
        calc_delta_nonbonded_energy(p, &mybead, dx.x, dx.y, dx.z, iwtype);
    if (delta_energy !=
        delta_energy) // isnan(delta_energy) ) not working with PGI OpenACC
      error = chain_index + 1;

    const soma_scalar_t old_mobility_modifier =
        get_mobility_modifier(p, iwtype, mybead.x, mybead.y, mybead.z);
    const soma_scalar_t new_mobility_modifier = get_mobility_modifier(
        p, iwtype, mybead.x + dx.x, mybead.y + dx.y, mybead.z + dx.z);
    const soma_scalar_t mobility_modifier =
        sqrt(old_mobility_modifier * new_mobility_modifier);
