...
unsigned int soma_mersenne_twister(MERSENNE_TWISTER_STATE *mt_rng) {
  ...
  if (mt_rng->internal_index >= MTMAX_num_int_state) {
    ...
    for (unsigned int i = 0; i < MTMAX_num_int_state - M; ++i) {
      h = (mt_rng->internal_state[i] & HI) |
          (mt_rng->internal_state[i + 1] & LO); // Crashes HERE!!!
      mt_rng->internal_state[i] =
          mt_rng->internal_state[i + M] ^ (h >> 1) ^ mt_rng->A[h & 1];
    }
    for (unsigned int i = MTMAX_num_int_state - M; i < MTMAX_num_int_state - 1;
         ++i) {
      h = (mt_rng->internal_state[i] & HI) |
          (mt_rng->internal_state[i + 1] & LO);
      mt_rng->internal_state[i] =
          mt_rng->internal_state[i + (M - MTMAX_num_int_state)] ^ (h >> 1) ^
          mt_rng->A[h & 1];
    }

    h = (mt_rng->internal_state[MTMAX_num_int_state - 1] & HI) |
        (mt_rng->internal_state[0] & LO);
...
unsigned int soma_rng_uint_max_mt() { return 0x80000000; }
...
unsigned int soma_rng_tt800(TT800STATE *itt800_rng) {
  ...
  if (itt800_rng->internal_index >= TT_num_int_state) {
    ...
    for (k = 0; k < TT_num_int_state - M; ++k) {
      const uint32_t tmp = (itt800_rng->internal_state[k] >> 1) ^
    ...
    for (k = TT_num_int_state - M; k < TT_num_int_state; ++k) {
      ...
      itt800_rng->internal_state[k] =
          itt800_rng->internal_state[k + (M - TT_num_int_state)] ^ tmp;
    }
