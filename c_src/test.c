    ...
    for (uint64_t i = 0; i < p->n_polymers; i++) {
      ...
      for (unsigned int j = 0; j < N; j++) {
        const uint64_t index =
            coord_to_index(p, beads[j].x, beads[j].y, beads[j].z);
        if (index >= p->n_cells_local) {
          fprintf(stderr,
                  "ERROR: domain Error %d %d. Particle out of domain %s:%d\n",
                  (int)i, j, __FILE__, __LINE__);
          int x, y, z;
          coord_to_cell_coordinate(p, beads[j].x, beads[j].y, beads[j].z, &x,
                                   &y, &z);
          fprintf(stderr, "\t %d %f %f %f\t %d %d %d\t %d %d\t %f %f %f\n",
                  p->info_MPI.world_rank, beads[j].x, beads[j].y, beads[j].z, x,
                  y, z, p->local_nx_low, p->local_nx_high, p->polymers[i].rcm.x,
    ...
    for (unsigned int set_id = 0; set_id < set->n_sets; set_id++) {
      ...
      for (unsigned int i = 0; i < set->set_length[set_id]; i++) {
        ...
        if (p->info_MPI.domain_rank == 0 && n_neigh > 0)
          fprintf(
              stderr,
              "ERROR: mono %d of poly_type %d has neighbors in its set %d.\n",
              pi, poly_type, set_id);
        ret += n_neigh;
      }
      // printf("\n");
...
int test_area51_exact(const struct Phase *const p) {
  ...
  if (p->area51 != NULL) {
    for (uint64_t i = 0; i < p->n_polymers; i++) {
      ...
      for (unsigned int j = 0; j < N - 1; j++) {
        ...
        dx.z = b.z - a.z;
        const bool ok =
            possible_move_area51(p, a.x, a.y, a.z, dx.x, dx.y, dx.z, false);
        if (!ok) {
...
#if (ENABLE_MPI == 1)
  MPI_Allreduce(MPI_IN_PLACE, &violations, 1, MPI_UNSIGNED, MPI_SUM,
  ...
  if (p->info_MPI.sim_rank == 0) {
    if (violations == 0)
      ...
      printf(
          "WARNING: At t= %d area51 exact test **FAILED** with %d violations. "
          "If area51 contains 'steps' this is not necessarily a problem.\n",
          p->time, violations);
  }
  return violations;
}
...
int test_chains_in_domain(struct Phase *const p) {
  ...
  if (p->info_MPI.sim_rank == 0) {
    if (violations == 0)
      ...
      printf("WARNING: At t= %d sim %d **FAILED** the domain chain test with "
             "%d violations.\n",
             p->time, p->info_MPI.world_rank / p->info_MPI.sim_rank,
             violations);
  }
  return violations;
}
...
int test_poly_conversion(struct Phase *const p) {
  ...
  for (unsigned int i = 0; i < p->pc.len_reactions; i++) {
    ...
    if (in >= p->n_poly_type) {
      fprintf(stderr,
              "ERROR: %d polytype input %d %d %d exceeds the number of known "
              "polymer types.\n",
    ...
    if (out >= p->n_poly_type) {
      fprintf(stderr,
              "ERROR: %d polytype output %d %d %d exceeds the number of known "
              "polymer types.\n",
    ...
    if (Nin != Nout) {
      fprintf(stderr,
              "ERROR: %d polytype conversion between to polymer types of "
              "different length. %i %d %d\n",
  ...
  if (p->pc.len_reactions > 0) {
    if (!p->pc.reaction_end[p->pc.len_reactions - 1]) {
      fprintf(stderr,
              "ERROR: %d the last element of the reaction list for the polymer "
              "conversion should have the end flag set.\n",
              p->info_MPI.sim_rank);
      return -3;
    }
  }
  ...
  if (maximum > p->pc.len_reactions) {
    fprintf(stderr,
            "ERROR: %d the reaction array contains values that are invalid to "
            "the corresponding list. %d %d\n",
