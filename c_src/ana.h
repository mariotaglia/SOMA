 ...
 *  structure factor calculated. It can be time dependent (dynamical) and time
 * independent(static)*/
enum structure_factor_type {
...
/*!>\brief calculate the variance between the current density field and the
  density field at a past time \param p Phase configuration to analyze \returns
  global \f$ dvar \f$ \param dvar pointer to return density variance
*/
...
/*!>\brief calculate the gyration radius for the polymers of the phase
  ...
  poly type.
  \returns global \f$ Rg^2 \f$ and its squared components independently of the
  polymer type
*/
...
/*!>\brief calculate the monomer and chain mean square displacement
  ...
  The order is: x-component of MSD, y-component of MSD,z-component of MSD, MSD
  in 3D and x-componenet of mass center MSD, y-componenet of mass center MSD,
  z-componenet of mass center MSD, mass center MSD.
*/
...
/*! \brief calculate bond anisotropy
  ...
  \param p Phase configuration to analyze.
  \param result Array to store the tensor. xx yy zz xy xz yz. Length of array
  must be 6*n_poly_types. \post the memory of where the pointers point is set to
...
//! \param acc_ratio Pointer to initialized soma_scalar_t where the result is
...
//! \param p System to analyze
//! \param non_bonded_energy Pointer to array of size p->n_types to store the
//! result
...
//! \param p System to analyze
//! \param bonded_energy Pointer to array of size NUMBER_SOMA_BOND_TYPES to
//! store the result
...
//! \brief Helper to ouput soma_scalar_t data to a hdf5 file, by extending an \a
//! existing dataset.
//!
...
//! \param sf_type Type of structure factor. 0 stands for dynamical, 1 stand for
//! static \return Errorcode.
...
//! \brief Helper to ouput soma_scalar_t data to a hdf5 file, used for the
//! strcture.
//!
//! \param p Pointer to the state of the system.
//! \param data Pointer to the data to ouput.
