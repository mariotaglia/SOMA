...
int set_openacc_devices(struct Phase *const p) {
  ...
  if (on_host) {
    ...
    if (my_gpu_rank >= num_gpus) {
      fprintf(
          stderr,
          "ERROR: rank %d tried to set gpuId %u, but only %u are available.\n",
    ...
    if (check_gpu != my_gpu_rank) {
      fprintf(stderr,
              "WARNING: rank %d tried to run GPU %u, but now it runs GPU %u.\n",
    ...
    ncclCommInitRank(&(p->info_MPI.SOMA_nccl_world), p->info_MPI.world_size, id,
                     p->info_MPI.world_rank);

    if (p->info_MPI.sim_rank == 0)
      ncclGetUniqueId(&id);
    MPI_Bcast(&id, sizeof(id), MPI_BYTE, 0, p->info_MPI.SOMA_comm_sim);
    ncclCommInitRank(&(p->info_MPI.SOMA_nccl_sim), p->info_MPI.sim_size, id,
                     p->info_MPI.sim_rank);

    if (p->info_MPI.domain_rank == 0)
      ncclGetUniqueId(&id);
    MPI_Bcast(&id, sizeof(id), MPI_BYTE, 0, p->info_MPI.SOMA_comm_domain);
    ncclCommInitRank(&(p->info_MPI.SOMA_nccl_domain), p->info_MPI.domain_size,
                     id, p->info_MPI.domain_rank);
#endif // ENABLE_MPI_CUDA
  }
  if (p->args.omp_threads_given && p->info_MPI.world_rank == 0) {
    fprintf(
        stderr,
        "ERROR: You passed an OMP option for an OPENACC compiled program. This"
...
#else //_OPENACC
  if ((p->args.only_gpu_given ||
       (p->args.gpus_given && p->args.gpus_arg != 0)) &&
    ...
    fprintf(stderr, "WARNING: The command line arguments request a GPU use,"
                    ...
                    "\t If you want GPU acceleration, recompile SOMA with "
                    "OpenACC support.\n"
  ...
  if (p->args.omp_threads_given && p->args.omp_threads_arg > 1) {
    ...
    if (p->args.omp_threads_arg > omp_get_num_procs()) {
      fprintf(stderr,
              "WARNING: world rank %d tried to use %d OMP threads, but only %d "
              "are available. Resetting to max. number.\n",
  ...
  if (p->args.omp_threads_given && p->args.omp_threads_arg > 1) {
    fprintf(stderr,
            "WARNING: world rank %d tried to use %d OMP threads, but the "
            "binary is compiled without OMP support.\n",
            p->info_MPI.world_rank, p->args.omp_threads_arg);
  }
#endif //_OPENMP
