...
do_not_edit_message = "//This file is autogenerated by the "+__file__+" script, do not modify this manually, as it will be lost.\n"
...
if __name__ == "__main__":
    ...
    # change directory to c_src. This assumes this script is located in SOMA/python_src
    # and c_src is also in SOMA
    ...
    print("total test-cases: "+str(sum(map( lambda kv : len(kv[1]), test_group_with_cases.items()))))
    print("to execute all tests, "+str(max_world_size)+" ranks will be required")

    with open("run_unit_tests.h", "w") as f:
        f.write(do_not_edit_message)
        f.write("void run_all_unit_tests(void);\n\n")

    with open("run_unit_tests.c", "w") as f:
        f.write(do_not_edit_message)
        f.write("#include \"run_unit_tests.h\"\n")
    ...
    if (warnings > 0):
        print("extracting test cases finished with "+str(warnings)+" warnings")
    sys.exit(warnings)
